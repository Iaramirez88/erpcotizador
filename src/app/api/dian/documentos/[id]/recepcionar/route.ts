  ...
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      const next = await tx.dianElectronicDocument.update({
        where: { id: doc.id },
        data: { status: DianDocumentStatus.RECEIVED, receivedAt: new Date(), lastError: null },
        select: { id: true, status: true, receivedAt: true },
      })

      await tx.dianElectronicEvent.create({
        data: { documentId: doc.id, type: DianEventType.RECEPTION, message: 'Recepción registrada.' },
      })
    ...
    console.error('Error al recepcionar documento DIAN:', error)
    return NextResponse.json({ ok: false, error: 'Error al recepcionar documento DIAN' }, { status: 500 })
  }
}
