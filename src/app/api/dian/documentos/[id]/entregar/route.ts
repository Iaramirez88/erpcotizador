  ...
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      const next = await tx.dianElectronicDocument.update({
        where: { id: doc.id },
        data: { status: DianDocumentStatus.DELIVERED, deliveredAt: new Date(), lastError: null },
        select: { id: true, status: true, deliveredAt: true },
      })

      await tx.dianElectronicEvent.create({
        data: { documentId: doc.id, type: DianEventType.DELIVERY, message: 'Entrega registrada.' },
      })
    ...
    console.error('Error al entregar documento DIAN:', error)
    return NextResponse.json({ ok: false, error: 'Error al entregar documento DIAN' }, { status: 500 })
  }
}
