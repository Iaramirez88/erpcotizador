...
export type InventoryDashboardView = 'stock' | 'movements'
...
export function InventoryDashboardPage({ view }: { view: InventoryDashboardView }) {
  ...
  async function load() {
    ...
    try {
      ...
      const [resMaterials, resMovements, resBodegas] = await Promise.all([
        fetch(`/api/materiales?search=${encodeURIComponent(search)}`),
        isMovementsView ? fetch(movementsUrl.toString()) : Promise.resolve(null),
        fetch("/api/bodegas"),
      ])

      const jsonMaterials = (await resMaterials.json().catch(() => ({}))) as { success?: boolean; data?: Material[] }
      const jsonMovements = resMovements
        ? (await resMovements.json().catch(() => ({}))) as { success?: boolean; data?: Movement[] }
        : null
      ...
      if (resMovements && !resMovements.ok) setError(t('inventory.errors.loadMovements'))
  ...
  }, [search, warehouseFilterId, isMovementsView])
  ...
  return (
    <div className="space-y-6">
      <ErpPageHero
        ...
        actions={
          <>
            {!isMovementsView && canManageInventory ? <Button variant="outline" onClick={exportExcel} disabled={isLoading}>
              <Download className="mr-2 h-4 w-4" />
              {t('inventory.actions.exportExcel')}
            </Button> : null}
            {!isMovementsView && canManageInventory ? <Button variant="outline" onClick={() => openLowStockOrder(lowStockMaterials)} disabled={isLoading || lowStockMaterials.length === 0}>
        ...
        stats={isMovementsView
          ? [
              ...
              { label: 'Productos', value: activeMaterials.length, hint: 'Base disponible para registrar movimientos', tone: 'neutral' },
            ]
          : [
              { label: 'Productos', value: activeMaterials.length, hint: 'Productos activos', tone: 'neutral' },
              { label: 'Stock bajo', value: lowStockMaterials.length, hint: 'Productos por reabastecer', tone: 'amber' },
              { label: 'Bodegas', value: bodegas.length, hint: 'Bodegas detectadas', tone: 'sky' },
            ]}
      />
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="border-b text-left text-gray-600">
                      <th className="w-12 py-2 pr-3">{t('inventory.stock.columns.image')}</th>
                        ...
                        <tr key={m.id} className="border-b last:border-b-0">
                          <td className="py-2 pr-3">
                            <img
                              ...
                              alt={m.nombre}
                              className="h-8 w-8 rounded border bg-white object-cover"
                              onError={(e) => {
                          ...
                          <td className={cn("py-2 pr-4", low ? "font-semibold text-red-700" : "text-gray-900")}>
        ...
        <Card>
          ...
          <CardContent>
            <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center">
              ...
              <select
                className="max-w-sm rounded-md border border-gray-200 px-3 py-2 text-sm"
                    ...
                    {movements.map((mv) => (
                      <tr key={mv.id} className="border-b last:border-b-0">
                        ...
                        <td className="py-2 pr-4">
                          <span className={cn("rounded px-2 py-1 text-xs font-semibold", movementBadgeClass(mv.type))}>
