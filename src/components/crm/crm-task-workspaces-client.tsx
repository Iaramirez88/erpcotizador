...
export function CrmTaskWorkspacesClient() {
  ...
  const taskFormWorkspace = useMemo(() => workspaces.find((workspace) => workspace.id === taskForm.workspaceId) ?? null, [taskForm.workspaceId, workspaces])
  ...
  const taskAssigneeCandidates = useMemo(() => {
    ...
    const selectedWorkspaceMemberIds = new Set((taskFormWorkspace?.members ?? []).map((member) => member.userId))
