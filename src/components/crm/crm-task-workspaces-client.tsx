...
function formatAttachmentSize(sizeBytes?: number | null) {
  if (!sizeBytes || !Number.isFinite(sizeBytes)) return 'Sin tamaÃ±o'
...
export function CrmTaskWorkspacesClient() {
  ...
  async function handleDeleteWorkspace(workspace: Workspace) {
    ...
    if (!window.confirm(`Se eliminarÃ¡ el espacio "${workspace.name}" si estÃ¡ vacÃ­o. Â¿Deseas continuar?`)) return
  ...
  async function handleDeleteProject(project: WorkspaceProject) {
    if (!selectedWorkspaceId) return alert('Selecciona primero un espacio de trabajo.')
    if (!window.confirm(`Se eliminarÃ¡ el proyecto "${project.name}" si no tiene tareas. Â¿Deseas continuar?`)) return
  ...
  async function handleCreateTask() {
    ...
    if (!taskForm.title.trim()) return alert('El tÃ­tulo de la tarea es requerido.')
    setSavingTask(true)
    try {
      ...
      toast({
        ...
        description: taskForm.assignedToUserIds.length
          ? 'La tarea se creÃ³ y se notificÃ³ a los responsables asignados.'
          : 'La tarea se creÃ³ correctamente en el espacio de trabajo.',
      })
    } finally {
      setSavingTask(false)
    }
  ...
  async function moveTaskToProject(task: TaskItem, workspaceId: string, projectId: string) {
    ...
    const updated = await handleUpdateTask(task.id, { workspaceId, projectId }, {
      title: 'Tarea movida',
      description: 'La tarea quedÃ³ ubicada en el proyecto seleccionado.',
    })
  ...
  async function handleMoveTask() {
    ...
    if (!task) return alert('La tarea ya no estÃ¡ disponible.')
  ...
  async function handleDeleteTask(task: TaskItem) {
    ...
    if (!window.confirm(`Se eliminarÃ¡ la tarea "${task.title}". Esta acciÃ³n no se puede deshacer.`)) return
    ...
    await loadTasks(selectedWorkspaceId)
    await loadBase()
    toast({ title: 'Tarea eliminada', description: 'La tarea se borrÃ³ del proyecto.' })
  }
    ...
    try {
      ...
      await handleUpdateTask(detailForm.id, patch, {
        ...
        description: detailForm.assignedToUserIds.length
          ? 'Se guardaron los cambios y se mantuvo la asignaciÃ³n de responsables.'
  ...
  async function handleAddNote() {
    ...
    try {
      ...
      toast({
        ...
        description: 'La nota quedÃ³ en el historial y se notificÃ³ a los responsables.',
      })
    } finally {
      setSavingNote(false)
    }
  ...
  async function handleQuickAddNote() {
    ...
    try {
      ...
      toast({
        title: 'Nota registrada',
        description: 'La nota se agregÃ³ y los responsables recibieron la novedad.',
      })
    } finally {
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" className="w-56 rounded-2xl p-2">
          {statusOptions.map((option) => (
            <DropdownMenuItem
              key={option.value}
              onSelect={() => void handleUpdateTask(task.id, { status: option.value }, { title: 'Estado actualizado', description: `La tarea quedÃ³ en estado ${option.label.toLowerCase()}.` })}
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" className="w-56 rounded-2xl p-2">
          {priorityOptions.map((option) => (
            <DropdownMenuItem
              key={option.value}
              onSelect={() => void handleUpdateTask(task.id, { priority: option.value }, { title: 'Prioridad actualizada', description: `La tarea quedÃ³ con prioridad ${option.label.toLowerCase()}.` })}
              className="rounded-xl px-2 py-1.5"
            >
              <span className={`inline-flex w-full items-center justify-between rounded-full border px-3 py-2 text-sm font-semibold ${option.className}`}>
                <span>{option.label}</span>
  ...
  function renderTaskCreatedAtColumn(task: TaskItem) {
    return (
      <div className="min-w-0">
        ...
        <p className="truncate text-xs text-slate-500">CreaciÃ³n de la tarea</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <ErpPageHero
        ...
        description="Administra espacios transversales del ERP, organiza tareas colaborativas, adjunta evidencia y centraliza seguimiento con estados mÃ¡s claros y visuales mÃ¡s fuertes."
        ...
        stats={[
          ...
          { label: 'En curso', value: filteredTasks.filter((task) => task.status === 'IN_PROGRESS' && !task.archivedAt).length, hint: 'EjecuciÃ³n activa', tone: 'teal' },
        ...
        {!workspacePanelCollapsed ? (
          <Card className="rounded-[26px] border-slate-200 shadow-[0_20px_40px_-32px_rgba(15,23,42,0.32)] xl:flex xl:max-h-[calc(100vh-12rem)] xl:flex-col xl:overflow-hidden">
            <CardHeader className="border-b border-slate-100 pb-5">
              ...
              <CardDescription>Selecciona un espacio, ajusta sus opciones desde el menÃº y crea tareas solo dentro de un proyecto.</CardDescription>
              <div className="relative mt-4">
                <SearchIcon className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                <Input
                  value={workspaceListSearch}
                  onChange={(event) => setWorkspaceListSearch(event.target.value)}
                  placeholder="Buscar espacio por nombre o descripciÃ³n"
            ...
            <CardContent className="min-h-0 space-y-3 p-4 md:p-5 xl:flex-1 xl:overflow-y-auto">
              ...
              {!loading && workspaces.length === 0 ? <p className="text-sm text-muted-foreground">No tienes espacios de trabajo todavÃ­a.</p> : null}
              {!loading && workspaces.length > 0 && filteredWorkspaces.length === 0 ? <p className="text-sm text-muted-foreground">No encontrÃ© espacios con ese criterio.</p> : null}
                      ...
                      <div className="mt-4 space-y-3 border-t border-sky-200/70 pt-4">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="text-xs text-slate-500">Cada proyecto ocupa todo el ancho y desde su botÃ³n + puedes crear tareas o gestionarlo.</p>
                          </div>
                          ...
                          {workspace.projects.map((project) => (
                            ...
                            >
                              ...
                              <div className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm text-slate-600">
                                <div className="min-w-0">
                                  <p className="truncate text-xs text-slate-700">{project.description || 'Sin descripciÃ³n operativa por ahora.'}</p>
                        ...
                        {!workspace.projects.length ? <p className="text-xs text-slate-500">Este espacio aÃºn no tiene proyectos. Crea uno para empezar a registrar tareas.</p> : null}
                        {selectedProjectId ? <p className="text-xs text-slate-500">La creaciÃ³n de tareas ahora se hace desde el botÃ³n + del proyecto seleccionado.</p> : null}
                      </div>
                    ) : null}
                  </div>
                )
          ...
          <CardHeader className="border-b border-slate-100 pb-5">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <CardDescription>
                  {showAllTasks
                    ? `Vista global con tareas de todos los espacios accesibles${selectedWorkspace ? ` Â· base actual ${selectedWorkspace.name}` : ''}`
                    : selectedWorkspace
                    ? `${selectedWorkspace.name}${selectedProject ? ` Â· Proyecto ${selectedProject.name}` : ' Â· Todos los proyectos'} Â· ${formatRole(selectedWorkspace.currentUserRole)}${selectedWorkspace.permissions?.canEditTasks ? ' con ediciÃ³n de tareas' : ' solo lectura'}`
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>Buscar por tarea, responsable, campo o descripciÃ³n</TooltipContent>
                    </Tooltip>
                    <Tooltip>
                      <TooltipTrigger asChild>
            ...
            <div className="w-full overflow-x-auto overscroll-x-contain pb-2">
              <div className="w-max" style={{ minWidth: `${taskTableMinWidth}px` }}>
                <div className="grid gap-3 border-b border-slate-100 px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500" style={{ gridTemplateColumns: taskGridTemplate }}>
                  ...
                  {visibleExtraTaskColumns.includes('history') ? <span>Ãšltimo cambio</span> : null}
                  {visibleExtraTaskColumns.includes('note') ? <span>Nota rÃ¡pida</span> : null}
                  <span>Acciones</span>
                </div>
                {filteredTasks.map((task) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="truncate text-slate-600">{task.description || 'Sin descripciÃ³n'}</p>
                      ...
                      <div className="flex items-center gap-1 overflow-hidden">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48 rounded-2xl p-1.5">
                            <DropdownMenuItem onSelect={() => void handleUpdateTask(task.id, { archived: !task.archivedAt }, { title: task.archivedAt ? 'Tarea restaurada' : 'Tarea archivada', description: task.archivedAt ? 'La tarea volviÃ³ a estar activa.' : 'La tarea se moviÃ³ a archivadas.' })} disabled={!canEditTasks}>
        ...
        <DialogContent className={quickTaskPanel?.mode === 'note' ? 'max-w-md' : 'max-w-2xl'}>
          <DialogHeader>
            <DialogTitle>
              ...
              {quickTaskPanel?.mode === 'history' ? 'Ãšltimo cambio' : null}
              {quickTaskPanel?.mode === 'note' ? 'Crear nota rÃ¡pida' : null}
            </DialogTitle>
            <DialogDescription>
              {quickTask ? quickTask.title : 'Selecciona una tarea vÃ¡lida.'}
            </DialogDescription>
          </DialogHeader>
                ...
                <div key={attachment.id} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-xs text-slate-500">{attachment.type.toUpperCase()} Â· {formatAttachmentSize(attachment.sizeBytes)}</p>
                    </div>
                  </div>
          ...
          {quickTaskPanel?.mode === 'history' ? (
            quickTaskLatestHistory ? (
              <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 text-sm text-slate-600">
                ...
                <p className="mt-2 text-xs text-slate-500">{quickTaskLatestHistory.actorUser?.name || quickTaskLatestHistory.actorUser?.email || 'Sistema'} Â· {quickTaskLatestHistory.type}</p>
              </div>
            ) : <p className="text-sm text-slate-500">Esta tarea no tiene historial todavÃ­a.</p>
          ) : null}
