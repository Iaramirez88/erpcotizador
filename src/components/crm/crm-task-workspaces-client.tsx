...
type TaskWorkspaceBootstrap = {
  workspaces: Workspace[];
  settings: TaskWorkspaceSettings;
};
...
> = {
  OPEN: {
    ...
    softClass: "from-slate-50 via-white to-slate-100 border-slate-200",
  },
  IN_PROGRESS: {
    label: "En curso",
    badgeClass: "bg-amber-400 text-slate-950 border-amber-300",
    softClass: "from-amber-50 via-white to-orange-100 border-amber-200",
  },
  DONE: {
    label: "Finalizada",
    badgeClass: "bg-emerald-500 text-white border-emerald-400",
    softClass: "from-emerald-50 via-white to-lime-100 border-emerald-200",
  },
  CANCELED: {
    label: "Cancelada",
    badgeClass: "bg-rose-500 text-white border-rose-400",
    softClass: "from-rose-50 via-white to-pink-100 border-rose-200",
  },
};
...
function getWorkspaceSedes(
  workspace?: Pick<Workspace, "sede" | "sedes"> | null,
) {
...
function getWorkspaceDescription(workspace: Workspace) {
  ...
  if (workspace.scope === "SEDE") {
    const names = getWorkspaceSedes(workspace)
      .map((sede) => sede.nombre)
      .filter(Boolean);
    return names.length ? names.join(", ") : "Espacio por sede";
  }
  return (
    workspace.ownerUser?.name ||
    workspace.ownerUser?.email ||
    "Espacio por usuario"
  );
}
...
function getTaskShareUserIds(args: {
  assigneeUserIds: string[];
  currentUserId?: string | null;
}) {
  return getUniqueIds(args.assigneeUserIds).filter(
...
function normalizeAttachments(value: unknown): TaskAttachment[] {
  ...
  value.forEach((item, index) => {
    ...
    const type = String(row.type || "")
      .trim()
      .toLowerCase() as TaskAttachmentType;
    if (
      !name ||
      !url ||
      !["image", "audio", "video", "document"].includes(type)
    )
      return null;
    attachments.push({
      ...
      provider:
        typeof row.provider === "string"
          ? (row.provider as TaskAttachment["provider"])
          : null,
...
function normalizeCustomFields(value: unknown): TaskCustomField[] {
  ...
  value.forEach((item, index) => {
    ...
    const type = String(row.type || "")
      .trim()
      .toUpperCase() as TaskCustomFieldType;
...
function normalizeTask(row: TaskItem): TaskItem {
  return {
    ...row,
    colorHex:
      typeof row.colorHex === "string" ? normalizeHex(row.colorHex) : null,
...
function buildTaskDetailSnapshot(
  detailForm: TaskDetailFormState,
  customFieldDraft: TaskCustomFieldDraftState,
) {
...
function getSortedTaskHistoryEntries(
  history: TaskHistoryEntry[] | null | undefined,
) {
...
function translateTaskHistoryMessage(message: string, type?: TaskHistoryType) {
  ...
  if (
    type === "UPDATED" &&
    normalized === "Se actualizaron los detalles de la tarea."
  ) {
...
const TASK_PRIORITY_COLUMN_STORAGE_KEY =
  "crm-task-workspaces:task-priority-column-visible";
const TASK_CREATED_AT_COLUMN_STORAGE_KEY =
  "crm-task-workspaces:task-created-at-column-visible";
...
function reorderValuesByPlacement(
  values: string[],
  fromValue: string,
  toValue: string,
...
export function CrmTaskWorkspacesClient() {
  ...
  const createCustomFieldFileInputRef = useRef<HTMLInputElement | null>(null);
  ...
  const [taskRelationOptionsOpen, setTaskRelationOptionsOpen] = useState(false);
  ...
  const [externalAttachmentDialogOpen, setExternalAttachmentDialogOpen] =
    useState(false);
  ...
  const [createTaskNote, setCreateTaskNote] = useState("");
  const [createTaskFiles, setCreateTaskFiles] = useState<File[]>([]);
  const [createTaskCustomFields, setCreateTaskCustomFields] = useState<
    CreateTaskCustomField[]
  >([]);
  const [createTaskCustomFieldDraft, setCreateTaskCustomFieldDraft] = useState({
    label: "",
    type: "TEXT" as TaskCustomFieldType,
    textValue: "",
    file: null as File | null,
  });
  const [detailHistoryVisibleCount, setDetailHistoryVisibleCount] = useState(5);
  const [quickTaskPanel, setQuickTaskPanel] = useState<{
    taskId: string;
    mode: QuickTaskPanelMode;
  } | null>(null);
  ...
  const [visibleExtraTaskColumns, setVisibleExtraTaskColumns] = useState<
    ExtraTaskColumn[]
  >([]);
  ...
  const [taskSettings, setTaskSettings] = useState<TaskWorkspaceSettings>({
    requireTaskCancellationReason: false,
  });
  const [workspaceViewportHeight, setWorkspaceViewportHeight] = useState<
    number | null
  >(null);
  ...
  const [taskCancelTarget, setTaskCancelTarget] = useState<TaskItem | null>(
    null,
  );
  const [taskCancelReason, setTaskCancelReason] = useState("");
  const [cancellingTask, setCancellingTask] = useState(false);
  const [customFieldUploadTarget, setCustomFieldUploadTarget] = useState<
    string | null
  >(null);
  ...
  const [taskMoveForm, setTaskMoveForm] = useState({
    taskId: "",
    workspaceId: "",
    projectId: "",
  });
  const [detailForm, setDetailForm] = useState<TaskDetailFormState>(
    getInitialTaskDetailForm,
  );
  const [customFieldDraft, setCustomFieldDraft] =
    useState<TaskCustomFieldDraftState>(getInitialCustomFieldDraft);
  ...
  const [externalAttachmentForm, setExternalAttachmentForm] = useState(
    getInitialExternalAttachmentForm(),
  );
  ...
  const taskFormWorkspace = useMemo(
    () =>
      workspaces.find((workspace) => workspace.id === taskForm.workspaceId) ??
      null,
  ...
  const manageableWorkspaces = useMemo(
    () => workspaces.filter((workspace) => workspace.permissions?.canManage),
    [workspaces],
  );
  const editableWorkspaces = useMemo(
    () => workspaces.filter((workspace) => workspace.permissions?.canEditTasks),
    [workspaces],
  );
  const editableWorkspaceIds = useMemo(
    () => new Set(editableWorkspaces.map((workspace) => workspace.id)),
    [editableWorkspaces],
  );
  const canCreateTask = Boolean(currentUserId);
  const selectedMoveWorkspace = useMemo(
    () =>
      workspaces.find(
        (workspace) => workspace.id === taskMoveForm.workspaceId,
      ) ?? null,
    [taskMoveForm.workspaceId, workspaces],
  );
  const clampedTaskColumnWidth = useMemo(
    () => Math.min(220, Math.max(120, taskColumnWidth)),
    [taskColumnWidth],
  );
  const showCrossWorkspaceColumn = taskViewMode !== "SPACE";
  const totalTaskColumnCount = useMemo(
    () =>
      7 +
      visibleExtraTaskColumns.length +
      (showCrossWorkspaceColumn ? 1 : 0) +
      (showPriorityColumn ? 1 : 0) +
      (showCreatedAtColumn ? 1 : 0),
    [
      showCreatedAtColumn,
      showCrossWorkspaceColumn,
      showPriorityColumn,
      visibleExtraTaskColumns.length,
    ],
  );
  const taskGridTemplate = useMemo(
    () => `repeat(${totalTaskColumnCount}, ${clampedTaskColumnWidth}px)`,
    [clampedTaskColumnWidth, totalTaskColumnCount],
  );
  const taskTableMinWidth = useMemo(
    () => clampedTaskColumnWidth * totalTaskColumnCount + 32,
    [clampedTaskColumnWidth, totalTaskColumnCount],
  );
  const shouldShowWorkspacePanel =
    workspaces.length > 0 && !workspacePanelCollapsed;
  const quickTask = useMemo(
    () =>
      quickTaskPanel
        ? (tasks.find((task) => task.id === quickTaskPanel.taskId) ?? null)
        : null,
    [quickTaskPanel, tasks],
  );
  const quickTaskLatestHistory = useMemo(
    () => getLatestTaskHistoryEntry(quickTask),
    [quickTask],
  );
  const selectedTaskHistory = useMemo(
    () => getSortedTaskHistoryEntries(selectedTask?.history),
    [selectedTask],
  );
  const visibleTaskHistoryEntries = useMemo(
    () => selectedTaskHistory.slice(0, detailHistoryVisibleCount),
    [detailHistoryVisibleCount, selectedTaskHistory],
  );
  const canLoadMoreTaskHistory =
    detailHistoryVisibleCount < selectedTaskHistory.length;
  const canCollapseTaskHistory =
    selectedTaskHistory.length > 5 && !canLoadMoreTaskHistory;
  const selectedTaskCanEdit = Boolean(
    selectedTask &&
      currentUserId &&
      (!selectedTask.workspace?.id ||
        editableWorkspaceIds.has(selectedTask.workspace.id)),
  );
  const quickTaskCanEdit = Boolean(
    quickTask &&
      currentUserId &&
      (!quickTask.workspace?.id ||
        editableWorkspaceIds.has(quickTask.workspace.id)),
  );
  const canEditTasks = Boolean(
    selectedWorkspace?.permissions?.canEditTasks ||
      selectedTaskCanEdit ||
      quickTaskCanEdit,
  );
  ...
  function queueDetailUrlCleanup() {
    ...
    const nextUrl = params.toString()
      ? `${resolvedPathname}?${params.toString()}`
      : resolvedPathname;
  ...
  async function loadBase() {
    setLoading(true);
    try {
      const [workspaceRes, userRes, sedeRes, meRes, uiPrefRes] =
        await Promise.all([
          ...
          requestJson<{
            report?: {
              tasks?: { pinnedTaskIds?: string[]; orderedTaskIds?: string[] };
            };
          }>("/api/ui-preferences"),
        ]);
      const nextWorkspaces = Array.isArray(workspaceRes.data?.workspaces)
        ? workspaceRes.data.workspaces
        : [];
      const lastWorkspaceId =
        typeof window !== "undefined"
          ? window.localStorage.getItem(LAST_WORKSPACE_STORAGE_KEY) || ""
          : "";
      setWorkspaces(nextWorkspaces);
      setTaskSettings(
        workspaceRes.data?.settings ?? { requireTaskCancellationReason: false },
      );
      setUsers(Array.isArray(userRes.data) ? userRes.data : []);
      setSedes(Array.isArray(sedeRes.data) ? sedeRes.data : []);
      setCurrentUserId(
        meRes.data?.id || nextWorkspaces[0]?.currentUserId || "",
      );
      setPinnedTaskIds(
        normalizePinnedTaskIds(uiPrefRes.data?.report?.tasks?.pinnedTaskIds),
      );
      setOrderedTaskIds(
        normalizeOrderedTaskIds(uiPrefRes.data?.report?.tasks?.orderedTaskIds),
      );
      setSelectedWorkspaceId((current) => {
        ...
        if (
          lastWorkspaceId &&
          nextWorkspaces.some((workspace) => workspace.id === lastWorkspaceId)
        ) {
  ...
  const loadTasks = useCallback(
    async (workspaceId = selectedWorkspaceId, viewMode = taskViewMode) => {
      if (
        (viewMode === "MINE" && !currentUserId) ||
        (viewMode === "SPACE" && !workspaceId)
      ) {
        setTasks([]);
        return;
      }
      ...
      const taskRes = await requestJson<TaskItem[]>(
        `/api/crm/tasks?${query.toString()}`,
      );
      setTasks(
        Array.isArray(taskRes.data)
          ? taskRes.data.map((row) => normalizeTask(row))
          : [],
      );
    },
  ...
  async function loadTaskDetail(taskId: string) {
    ...
    if (row) {
      ...
      const nextDetailForm: TaskDetailFormState = {
        ...
        assignedToUserIds: row.assignments.map(
      ...
      setCustomFieldUploadTarget(null);
      setDetailHasUnsavedChanges(false);
      lastPersistedDetailSnapshotRef.current = buildTaskDetailSnapshot(
        nextDetailForm,
  ...
  }, [loadTasks, selectedWorkspaceId, taskViewMode]);
  ...
  useEffect(() => {
    ...
    setDetailHasUnsavedChanges(
      buildTaskDetailSnapshot(detailForm, customFieldDraft) !==
        lastPersistedDetailSnapshotRef.current,
    );
  ...
  useEffect(() => {
    ...
    try {
      const savedWorkspacePanelState = window.localStorage.getItem(
        WORKSPACE_PANEL_STORAGE_KEY,
      );
      if (
        savedWorkspacePanelState === "true" ||
        savedWorkspacePanelState === "false"
      ) {
      ...
      const savedExtraColumns = JSON.parse(
        window.localStorage.getItem(TASK_EXTRA_COLUMNS_STORAGE_KEY) || "[]",
      ) as unknown;
      if (Array.isArray(savedExtraColumns)) {
        setVisibleExtraTaskColumns(
          savedExtraColumns.filter(
            (item): item is ExtraTaskColumn =>
              item === "attachments" ||
              item === "custom-fields" ||
              item === "history" ||
              item === "note" ||
      ...
      const savedPriorityColumn = window.localStorage.getItem(
        TASK_PRIORITY_COLUMN_STORAGE_KEY,
      );
      if (savedPriorityColumn === "true" || savedPriorityColumn === "false") {
        setShowPriorityColumn(savedPriorityColumn === "true");
      }
      const savedCreatedAtColumn = window.localStorage.getItem(
        TASK_CREATED_AT_COLUMN_STORAGE_KEY,
      );
      ...
      const savedPageSize = Number(
        window.localStorage.getItem(TASK_PAGE_SIZE_STORAGE_KEY) || "",
      );
      if ([10, 20, 30, 50].includes(savedPageSize)) {
        setTaskPageSize(savedPageSize);
      }
      const savedPeriodFilter = window.localStorage.getItem(
        TASK_PERIOD_FILTER_STORAGE_KEY,
      );
      if (
        savedPeriodFilter === "ALL" ||
        savedPeriodFilter === "WEEK" ||
        savedPeriodFilter === "MONTH" ||
        savedPeriodFilter === "QUARTER"
      ) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      window.localStorage.setItem(
        ...
        String(clampedTaskColumnWidth),
      );
      window.localStorage.setItem(
        TASK_EXTRA_COLUMNS_STORAGE_KEY,
        JSON.stringify(visibleExtraTaskColumns),
      );
      window.localStorage.setItem(
        TASK_PRIORITY_COLUMN_STORAGE_KEY,
        String(showPriorityColumn),
      );
      window.localStorage.setItem(
        TASK_CREATED_AT_COLUMN_STORAGE_KEY,
        String(showCreatedAtColumn),
      );
      window.localStorage.setItem(
        TASK_PAGE_SIZE_STORAGE_KEY,
        String(taskPageSize),
      );
      window.localStorage.setItem(
        TASK_PERIOD_FILTER_STORAGE_KEY,
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      setWorkspaceViewportHeight(
        Math.max(520, Math.floor(viewportHeight - top - 20)),
      );
    };
  ...
  useEffect(() => {
    return () => {
      if (
        notificationCleanupTimerRef.current &&
        typeof window !== "undefined"
      ) {
        window.clearTimeout(notificationCleanupTimerRef.current);
      }
  ...
  useEffect(() => {
    ...
    setWorkspaceSettingsForm({
      ...
      members: selectedWorkspace.members.map((member) => ({
  ...
  useEffect(() => {
    ...
    setSelectedProjectId((current) =>
      current &&
      selectedWorkspace.projects.some((project) => project.id === current)
        ? current
        : "",
    );
  }, [selectedWorkspace]);
  ...
  useEffect(() => {
    ...
    setTaskForm((current) =>
      current.assignedToUserIds.includes(currentUserId)
        ? current
        : {
            ...current,
            assignedToUserIds: [...current.assignedToUserIds, currentUserId],
          },
    );
  }, [currentUserId]);
  ...
  const filteredTasks = useMemo(() => {
    ...
    const projectScopedTasks =
      taskViewMode === "SPACE" && selectedProjectId
    ...
    const matchingTasks = !term
      ...
      : projectScopedTasks.filter((task) => {
          const haystack = [
            ...
            ...(task.customFieldsJson || []).map(
              (field) =>
                `${field.label} ${field.textValue || field.file?.name || ""}`,
            ),
          ]
            .filter(Boolean)
            .join(" ")
    ...
    const periodScopedTasks = matchingTasks.filter((task) =>
      isTaskInCurrentPeriod(task, taskPeriodFilter),
    );

    const pinnedVisibleIds = pinnedTaskIds.filter((taskId) =>
      periodScopedTasks.some((task) => task.id === taskId),
    );
    const pinnedIndex = new Map(
      pinnedVisibleIds.map((taskId, index) => [taskId, index]),
    );
    const orderedVisibleIds = orderedTaskIds.filter(
      (taskId) =>
        periodScopedTasks.some((task) => task.id === taskId) &&
        !pinnedVisibleIds.includes(taskId),
    );
    const orderedIndex = new Map(
      orderedVisibleIds.map((taskId, index) => [taskId, index]),
    );
  ...
  const visibleTaskPages = useMemo(() => {
    if (totalTaskPages <= 5)
      return Array.from({ length: totalTaskPages }, (_, index) => index + 1);
  ...
  async function savePinnedTasks(nextPinnedTaskIds: string[]) {
    ...
    const json = await requestJson<{
      ...
      body: JSON.stringify({
        report: { tasks: { pinnedTaskIds: nextPinnedTaskIds, orderedTaskIds } },
      }),
    });
    if (!json.success) {
      throw new Error(
        json.error || "No se pudieron guardar las tareas ancladas.",
      );
    }
  }
  ...
  async function saveOrderedTasks(nextOrderedTaskIds: string[]) {
    ...
    const json = await requestJson<{
      ...
      body: JSON.stringify({
        report: {
          tasks: { pinnedTaskIds, orderedTaskIds: nextOrderedTaskIds },
        },
      }),
    });
    if (!json.success) {
      throw new Error(
        json.error || "No se pudo guardar el orden manual de tareas.",
      );
    }
  }
  ...
  async function toggleTaskPinned(task: TaskItem) {
    ...
    const nextPinnedTaskIds = pinned
      ...
      : [
          ...filteredTasks
            .filter((item) => pinnedTaskIds.includes(item.id))
            .map((item) => item.id),
          task.id,
        ].sort((leftId, rightId) => {
    ...
    try {
      ...
      toast({
        ...
        description: pinned
          ? "La tarea volvió al flujo normal."
          : "La tarea quedó fija al inicio de tu vista.",
      });
    } catch (error) {
      ...
      alert(
        error instanceof Error
          ? error.message
          : "No se pudo actualizar el anclaje.",
      );
    }
  }
  ...
  async function reorderPinnedTasks(
    ...
    const nextPinnedTaskIds = reorderValuesByPlacement(
      pinnedTaskIds,
      fromTaskId,
      toTaskId,
      placement,
    ...
    try {
      ...
      alert(
        error instanceof Error
          ? error.message
          : "No se pudo reordenar el anclaje.",
      );
    }
  }
  ...
  async function reorderVisibleTasks(
    ...
    if (
      !visibleUnpinnedIds.includes(fromTaskId) ||
      !visibleUnpinnedIds.includes(toTaskId)
    )
      return;

    const normalizedCurrentOrder = [
      ...orderedTaskIds.filter((taskId) => visibleUnpinnedIds.includes(taskId)),
      ...visibleUnpinnedIds.filter(
        (taskId) => !orderedTaskIds.includes(taskId),
      ),
    ];

    const nextVisibleOrder = reorderValuesByPlacement(
    ...
    const preserved = orderedTaskIds.filter(
      (taskId) => !visibleSet.has(taskId),
    );
  ...
  const workspaceMemberCandidates = useMemo(() => {
    ...
    const selectedIds = new Set(
      workspaceSettingsForm.members.map((member) => member.userId),
    );
  ...
  const taskAssigneeCandidates = useMemo(() => {
    ...
    const selectedWorkspaceMemberIds = new Set(
      (taskFormWorkspace?.members ?? []).map((member) => member.userId),
    );
    const restrictToWorkspaceMembers = Boolean(taskForm.workspaceId);
    return users.filter((user) => {
      if (
        restrictToWorkspaceMembers &&
        !selectedWorkspaceMemberIds.has(user.id)
      )
        return false;
      return (
        !term ||
  ...
  }, [users, assigneeSearch, taskFormWorkspace, taskForm.workspaceId]);

  const detailAssigneeCandidates = useMemo(() => {
    const term = detailAssigneeSearch.trim().toLowerCase();
    const detailWorkspaceMemberIds = new Set(
      (selectedTask?.workspace?.members ?? []).map((member) => member.userId),
    );
    const restrictToWorkspaceMembers = Boolean(selectedTask?.workspace?.id);
    return users.filter((user) => {
      if (restrictToWorkspaceMembers && !detailWorkspaceMemberIds.has(user.id))
        return false;
      return (
        !term ||
        (user.name || "").toLowerCase().includes(term) ||
  ...
  const noteEntries = useMemo(
    () =>
      selectedTask?.history.filter((entry) => entry.type === "NOTE_ADDED") ??
      [],
    [selectedTask],
  );
  ...
  function openTaskCreationDialog(projectId = selectedProjectId) {
    ...
    setTaskRelationOptionsOpen(Boolean(workspaceId || projectId));
    setTaskForm({
      title: "",
      description: "",
      dueAt: "",
    ...
    setCreateTaskFiles([]);
    setCreateTaskCustomFields([]);
    setCreateTaskCustomFieldDraft({
      label: "",
      type: "TEXT",
      textValue: "",
      file: null,
  ...
  function handleAddCreateTaskCustomField() {
    const label = createTaskCustomFieldDraft.label.trim();
    if (!label) return alert("Escribe un nombre para el campo personalizado.");
    if (
      createTaskCustomFieldDraft.type === "TEXT" &&
      !createTaskCustomFieldDraft.textValue.trim()
    ) {
      return alert("Completa el contenido del campo de texto.");
    }
    if (
      createTaskCustomFieldDraft.type === "FILE" &&
      !createTaskCustomFieldDraft.file
    ) {
      return alert("Selecciona un archivo para este campo personalizado.");
    }

    setCreateTaskCustomFields((current) => [
      ...current,
      {
        id: createId("field"),
        label,
        type: createTaskCustomFieldDraft.type,
        textValue:
          createTaskCustomFieldDraft.type === "TEXT"
            ? createTaskCustomFieldDraft.textValue.trim()
            : "",
    ...
    if (createCustomFieldFileInputRef.current) {
      createCustomFieldFileInputRef.current.value = "";
    }
  }
  ...
  async function uploadFileForTask(taskId: string, file: File) {
    ...
    const response = await fetch(`/api/crm/tasks/${taskId}/attachments`, {
      method: "POST",
      body: formData,
    });
  ...
  function moveDetailTaskToSelectedProject() {
    if (!selectedProjectId)
      return alert(
        "Selecciona un proyecto de destino en el panel del espacio.",
      );
  ...
  async function handleCreateWorkspace() {
    if (!workspaceForm.name.trim())
      return alert("El nombre del proyecto es requerido.");
    if (workspaceForm.scope === "SEDE" && !workspaceForm.sedeIds.length)
      return alert("Selecciona al menos una sede para el proyecto.");
    if (workspaceForm.scope === "USER" && !workspaceForm.ownerUserId)
      return alert("Selecciona un usuario responsable del proyecto.");
    ...
    if (shareConfirmationMessage && !window.confirm(shareConfirmationMessage))
      return;
    ...
    try {
      ...
      if (!json.success || !json.data)
        return alert(json.error || "No se pudo crear el proyecto.");
  ...
  async function handleSaveWorkspaceSettings() {
    ...
    if (!canManageWorkspace)
      return alert("Solo un manager puede administrar miembros y roles.");
    if (!workspaceSettingsForm.name.trim())
      return alert("El nombre del proyecto es requerido.");
    if (
      workspaceSettingsForm.scope === "SEDE" &&
      !workspaceSettingsForm.sedeIds.length
    )
      return alert("Selecciona al menos una sede para el proyecto.");
    const nextSharedUserIds = getWorkspaceShareUserIds({
      ownerUserId: workspaceSettingsForm.ownerUserId,
      memberUserIds: workspaceSettingsForm.members.map(
    ...
    const previousSharedUserIds = getWorkspaceShareUserIds({
      ownerUserId: selectedWorkspace?.ownerUser?.id,
      memberUserIds:
        selectedWorkspace?.members.map((member) => member.userId) ?? [],
      currentUserId,
    });
    const shareConfirmationMessage = !areStringSetsEqual(
    ...
    setSavingWorkspace(true);
    try {
      const json = await requestJson<Workspace>(
        `/api/crm/task-workspaces/${workspaceSettingsForm.id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            visibility: workspaceSettingsForm.visibility,
            ownerUserId: workspaceSettingsForm.ownerUserId || null,
            sedeIds:
              workspaceSettingsForm.scope === "SEDE"
                ? workspaceSettingsForm.sedeIds
                : [],
      ...
      if (!json.success || !json.data)
        return alert(json.error || "No se pudo actualizar el proyecto.");
  ...
  async function handleDeleteWorkspace(workspace: Workspace) {
    if (!workspace.permissions?.canManage)
      return alert("Solo un manager puede eliminar este proyecto.");
    if (
      !window.confirm(
        `Se eliminará el proyecto "${workspace.name}" si está vacío. ¿Deseas continuar?`,
      )
    )
      return;
    const json = await requestJson<null>(
      `/api/crm/task-workspaces/${workspace.id}`,
      { method: "DELETE" },
    );
    if (!json.success)
      return alert(json.error || "No se pudo eliminar el proyecto.");
    await loadBase();
  }

  async function handleDeleteProject(project: WorkspaceProject) {
    if (!selectedWorkspaceId)
      return alert("Selecciona primero un espacio de trabajo.");
    if (
      !window.confirm(
        `Se eliminará el proyecto "${project.name}" si no tiene tareas. ¿Deseas continuar?`,
      )
    )
      return;
    const json = await requestJson<null>(
      `/api/crm/task-workspaces/${selectedWorkspaceId}/projects/${project.id}`,
      { method: "DELETE" },
    );
    if (!json.success)
      return alert(json.error || "No se pudo eliminar el proyecto.");
  ...
  async function moveProjectToWorkspace(
    ...
    if (!targetWorkspaceId || targetWorkspaceId === project.workspaceId)
      return false;
    const json = await requestJson<WorkspaceProject>(
      `/api/crm/task-workspaces/${project.workspaceId}/projects/${project.id}`,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    toast({
      title: "Lista movida",
      description: "La lista y sus tareas quedaron en el nuevo proyecto.",
    });
    return true;
  }
  ...
  async function handleSaveProject() {
    ...
    try {
      ...
      const json = await requestJson<WorkspaceProject>(url, {
        method,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!json.success || !json.data)
        return alert(
          json.error ||
            `No se pudo ${isEditing ? "actualizar" : "crear"} el proyecto.`,
        );
  ...
  async function handleCreateTask() {
    if (!taskForm.title.trim())
      return alert("El título de la tarea es requerido.");
    const normalizedAssigneeIds = Array.from(
      new Set([
        ...(currentUserId ? [currentUserId] : []),
        ...taskForm.assignedToUserIds,
      ]),
    );
    const sharedUserIds = getTaskShareUserIds({
    ...
    try {
      const json = await requestJson<TaskItem>("/api/crm/tasks", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          workspaceId: taskForm.workspaceId || null,
          ...
          colorHex: normalizeHex(taskForm.colorHex),
          assignedToUserIds: normalizedAssigneeIds,
        }),
      });
      if (!json.success)
        return alert(json.error || "No se pudo crear la tarea.");
      if (json.data?.id) {
        const nextOrderedTaskIds = [
          json.data.id,
          ...orderedTaskIds.filter((taskId) => taskId !== json.data?.id),
        ];
        try {
          await saveOrderedTasks(nextOrderedTaskIds);
        } catch {
        ...
        try {
          const uploadedAttachments = await Promise.all(
            createTaskFiles.map((file) => uploadFileForTask(json.data!.id, file)),
          );
          const customFieldsJson: TaskCustomField[] = await Promise.all(
            createTaskCustomFields.map(async (field) => ({
              id: field.id,
              label: field.label,
              type: field.type,
              textValue: field.type === "TEXT" ? field.textValue : null,
              file:
                field.type === "FILE" && field.file
                  ? await uploadFileForTask(json.data!.id, field.file)
                  : null,
            })),
          );

          if (uploadedAttachments.length || customFieldsJson.length) {
            const extrasResult = await requestJson<TaskItem>(
              `/api/crm/tasks/${json.data.id}`,
              {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  attachmentsJson: uploadedAttachments,
                  customFieldsJson,
                }),
              },
            );
            if (!extrasResult.success) {
              throw new Error(
                extrasResult.error || "No se pudieron guardar los datos adicionales.",
              );
            }
          }

          if (createTaskNote.trim()) {
            const noteResult = await requestJson<TaskItem>(
              `/api/crm/tasks/${json.data.id}/notes`,
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ note: createTaskNote.trim() }),
              },
            );
            if (!noteResult.success) {
              throw new Error(noteResult.error || "No se pudo guardar la nota inicial.");
            }
          }
        } catch (error) {
          alert(
            error instanceof Error
              ? `La tarea se creó, pero ${error.message}`
              : "La tarea se creó, pero no se pudieron guardar todos los datos adicionales.",
          );
        }
      }
      const shouldOpenDetailAfterCreate = Boolean(json.data?.id);
  ...
  async function moveTaskToProject(
    ...
    if (task.workspace?.id === workspaceId && task.project?.id === projectId)
      return false;
    const updated = await handleUpdateTask(
      ...
      { workspaceId, projectId },
      {
        ...
        description: projectId
          ? "La tarea quedó ubicada en la lista seleccionada."
          : "La tarea quedó relacionada al proyecto seleccionado.",
      },
    );
  ...
  async function handleMoveTask() {
    ...
    if (!taskMoveForm.workspaceId)
      return alert("Selecciona un proyecto de destino.");
    setMovingTask(true);
    try {
      const moved = await moveTaskToProject(
        task,
        taskMoveForm.workspaceId,
        taskMoveForm.projectId,
      );
      if (!moved) return;
  ...
  async function handleDeleteTask(task: TaskItem) {
    if (!canEditTask(task))
      return alert("Solo un editor o administrador puede eliminar tareas.");
    if (task.createdBy?.id !== currentUserId)
      return alert("Solo puedes eliminar tareas creadas por tu usuario.");
  ...
  async function handleConfirmTaskCancellation() {
    ...
    if (
      taskSettings.requireTaskCancellationReason &&
      !taskCancelReason.trim()
    ) {
    ...
    try {
      const json = await requestJson<TaskItem>(
        `/api/crm/tasks/${taskCancelTarget.id}`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
      ...
      if (!json.success)
        return alert(json.error || "No se pudo anular la tarea.");
  ...
  function handleProjectDragStart(
    ...
    event.dataTransfer.setData(
      "text/plain",
      serializeDragPayload({ type: "project", projectId: project.id }),
    );
  }
  ...
  function handleTaskDragStart(
    ...
    event.dataTransfer.setData(
      "text/plain",
      serializeDragPayload({ type: "task", taskId: task.id }),
    );
  }

  async function handleWorkspaceDrop(
    targetWorkspaceId: string,
    event: React.DragEvent<HTMLDivElement>,
  ) {
  ...
  async function handleTaskRowDrop(
    targetTask: TaskItem,
    event: React.DragEvent<HTMLDivElement>,
  ) {
    event.preventDefault();
    ...
    const draggedTask =
      tasks.find((item) => item.id === payload.taskId) ?? null;
    if (!draggedTask || !canEditTask(draggedTask)) return;

    if (isTaskPinned(payload.taskId) && isTaskPinned(targetTask.id)) {
      await reorderPinnedTasks(
        payload.taskId,
        targetTask.id,
        dragOverTaskPlacement,
      );
      return;
    }

    if (!isTaskPinned(payload.taskId) && !isTaskPinned(targetTask.id)) {
      await reorderVisibleTasks(
        payload.taskId,
        targetTask.id,
        dragOverTaskPlacement,
      );
  ...
  async function handlePinnedTaskDrop(
    targetTaskId: string,
    event: React.DragEvent<HTMLDivElement>,
  ) {
    event.preventDefault();
  ...
  async function handleSaveDetail() {
    ...
    if (selectedTask && !canEditTask(selectedTask))
      return alert("Tu rol actual no permite editar esta tarea.");
    const nextSharedUserIds = getTaskShareUserIds({
      assigneeUserIds: detailForm.assignedToUserIds,
      currentUserId,
    });
    const previousSharedUserIds = getTaskShareUserIds({
      assigneeUserIds:
        selectedTask?.assignments.map((assignment) => assignment.userId) ?? [],
      currentUserId,
    });
    const shareConfirmationMessage = !areStringSetsEqual(
      previousSharedUserIds,
      ...
      ? buildTaskShareConfirmationMessage({
          taskTitle:
            detailForm.title.trim() || selectedTask?.title || "Sin título",
          users,
          sharedUserIds: nextSharedUserIds,
          isEditing: true,
        })
    ...
    try {
      const patch: Record<string, unknown> = {
        ...
        colorHex: normalizeHex(detailForm.colorHex),
        attachmentsJson: detailForm.attachmentsJson,
        customFieldsJson: detailForm.customFieldsJson,
      ...
      if (
        ...
        detailForm.status === "CANCELED"
      ) {
        const reason = requestCancellationReason(
          detailForm.title || selectedTask.title,
          taskSettings.requireTaskCancellationReason,
        );
  ...
  async function handleAddNote() {
    if (!selectedTask || !noteDraft.trim())
      return alert("Escribe una nota para registrar en el historial.");
    if (!canEditTask(selectedTask))
      return alert("Tu rol actual no permite editar esta tarea.");
    setSavingNote(true);
    try {
      const json = await requestJson<TaskItem>(
        `/api/crm/tasks/${selectedTask.id}/notes`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (!json.success || !json.data)
        return alert(json.error || "No se pudo agregar la nota.");
      ...
      toast({
        ...
        description:
          "La nota quedó en el historial y se notificó a los responsables.",
      });
    } finally {
      setSavingNote(false);
    }
  ...
  async function handleQuickAddNote() {
    if (!quickTask || !quickNoteDraft.trim())
      return alert("Escribe una nota para registrar en el historial.");
    if (!canEditTask(quickTask))
      return alert("Tu rol actual no permite editar esta tarea.");
    setSavingQuickNote(true);
    try {
      const json = await requestJson<TaskItem>(
        `/api/crm/tasks/${quickTask.id}/notes`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ note: quickNoteDraft }),
        },
      );
      if (!json.success || !json.data)
        return alert(json.error || "No se pudo agregar la nota.");
      ...
      toast({
        title: "Nota registrada",
        description:
          "La nota se agregó y los responsables recibieron la novedad.",
      });
    } finally {
  ...
  async function uploadAttachment(file: File) {
    ...
    try {
      ...
      const response = await fetch(
        `/api/crm/tasks/${detailForm.id}/attachments`,
        { method: "POST", body: formData },
      );
      const json = (await response
        .json()
        .catch(() => ({}))) as JsonResponse<TaskAttachment>;
      if (!json.success || !json.data) {
      ...
      if (customFieldFileInputRef.current)
        customFieldFileInputRef.current.value = "";
    }
  }
  ...
  async function handleAttachmentFile(file: File | null) {
    ...
    setDetailForm((current) => ({
      ...current,
      attachmentsJson: [...current.attachmentsJson, uploaded],
    }));
  }
  ...
  async function handleLibraryAttachment(item: CrmFileItem) {
    if (!item.url) {
      alert(
        "Solo puedes vincular archivos existentes de la biblioteca, no carpetas.",
      );
      return;
    }
    const attachment = mapLibraryItemToAttachment(item);
    setDetailForm((current) =>
      current.attachmentsJson.some(
        (existing) => existing.url === attachment.url,
      )
        ? current
        : {
            ...current,
            attachmentsJson: [...current.attachmentsJson, attachment],
          },
    );
  }
  ...
  async function handleExternalAttachmentLink() {
    ...
    try {
      ...
      setDetailForm((current) =>
        current.attachmentsJson.some(
          (existing) => existing.url === json.data?.url,
        )
          ? current
          : {
              ...current,
              attachmentsJson: [
                ...current.attachmentsJson,
                json.data as TaskAttachment,
              ],
            },
      );
      setExternalAttachmentForm(getInitialExternalAttachmentForm());
    ...
    if (customFieldUploadTarget && customFieldUploadTarget !== "new") {
      setDetailForm((current) => ({
        ...
        customFieldsJson: current.customFieldsJson.map((field) =>
          field.id === customFieldUploadTarget
            ? { ...field, file: uploaded }
            : field,
        ),
      }));
  ...
  function handleAddCustomField() {
    ...
    if (!label) return alert("Escribe un nombre para el campo personalizado.");
    if (customFieldDraft.type === "TEXT" && !customFieldDraft.textValue.trim())
      return alert("Completa el contenido del campo de texto.");
    if (customFieldDraft.type === "FILE" && !customFieldDraft.file)
      return alert("Sube un archivo para este campo personalizado.");
    setDetailForm((current) => ({
      ...current,
      customFieldsJson: [
        ...current.customFieldsJson,
        {
          ...
          textValue:
            customFieldDraft.type === "TEXT"
              ? customFieldDraft.textValue.trim()
              : null,
  ...
  function renderTaskStatusControl(task: TaskItem) {
    ...
    const statusOptions: Array<{
    ...
    }> = [
      {
        value: "OPEN",
        label: "No iniciado",
        className: STATUS_META.OPEN.badgeClass,
      },
      {
        value: "IN_PROGRESS",
        label: "En curso",
        className: STATUS_META.IN_PROGRESS.badgeClass,
      },
      {
        value: "DONE",
        label: "Finalizada",
        className: STATUS_META.DONE.badgeClass,
      },
      {
        value: "CANCELED",
        label: "Cancelada",
        className: STATUS_META.CANCELED.badgeClass,
      },
    ];
    ...
    if (!canEditTasks) {
      return (
        <span
          className={`inline-flex w-fit rounded-full border px-3 py-1 text-xs font-semibold ${statusMeta.badgeClass}`}
        >
          {statusMeta.label}
        </span>
      );
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button
            ...
            style={{
              width: `${clampedTaskColumnWidth}px`,
              maxWidth: `${clampedTaskColumnWidth}px`,
            }}
        ...
        <DropdownMenuContent align="start" className="w-56 rounded-2xl p-2">
          {statusOptions.map((option) => (
            <DropdownMenuItem
              ...
              onSelect={() => {
                if (option.value === "CANCELED" && task.status !== "CANCELED") {
                  ...
                  if (taskSettings.requireTaskCancellationReason && !reason) {
                    alert(
                      "Debes registrar el motivo de anulación para continuar.",
                    );
                    return;
                  }
                  void handleUpdateTask(
                    ...
                    },
                    {
                      title: "Tarea anulada",
                      description: reason
                        ? "La anulación quedó registrada con motivo."
                        : "La tarea quedó anulada y archivada.",
                    },
                  );
                  return;
                }
                void handleUpdateTask(
                  ...
                  { status: option.value },
                  {
                    ...
                    description: `La tarea quedó en estado ${option.label.toLowerCase()}.`,
                  },
                );
              }}
            ...
            >
              <span
                className={`inline-flex w-full items-center justify-between rounded-full border px-3 py-2 text-sm font-semibold ${option.className}`}
              >
  ...
  function renderTaskPriorityControl(task: TaskItem) {
    ...
    const priorityOptions: Array<{
      ...
      { value: "LOW", label: "Baja", className: PRIORITY_META.LOW.badgeClass },
      {
        value: "NORMAL",
        label: "Normal",
        className: PRIORITY_META.NORMAL.badgeClass,
      },
      {
        value: "HIGH",
        label: "Alta",
        className: PRIORITY_META.HIGH.badgeClass,
      },
    ];
    ...
    if (!canEditTasks) {
      return (
        <span
          className={`inline-flex w-fit rounded-full border px-3 py-1 text-xs font-semibold ${priorityMeta.badgeClass}`}
        >
          {priorityMeta.label}
        </span>
      );
              ...
              onSelect={() =>
                void handleUpdateTask(
                  ...
                  { priority: option.value },
                  {
                    ...
                    description: `La tarea quedó con prioridad ${option.label.toLowerCase()}.`,
                  },
                )
              }
              className="rounded-xl px-2 py-1.5"
      ...
      <div className="min-w-0">
        ...
        <p className="truncate text-xs text-slate-500">
          {firstField.type === "TEXT"
            ? firstField.textValue || "Sin valor"
            : firstField.file?.name || "Archivo"}
        </p>
      </div>
    );
  }
  ...
  function renderTaskHistoryColumn(task: TaskItem) {
    ...
    return (
      <div className="min-w-0">
        <p className="truncate text-xs font-medium text-slate-700">
          {translateTaskHistoryMessage(
            latestHistory.message,
            latestHistory.type,
          )}
        </p>
        <p className="truncate text-xs text-slate-500">
          {formatDate(latestHistory.createdAt, "Sin fecha")}
        </p>
      </div>
    );
  }
  ...
  function renderTaskCreatedAtColumn(task: TaskItem) {
    return (
      <div className="min-w-0">
        <p className="truncate text-xs font-medium text-slate-700">
          {formatDate(task.createdAt, "Sin fecha")}
        </p>
  ...
  function renderTaskNoteColumn(task: TaskItem) {
    const notesCount = task.history.filter(
      (entry) => entry.type === "NOTE_ADDED",
    ).length;

    return (
      <div className="flex items-center justify-between gap-2 overflow-hidden">
        <span className="truncate text-xs text-slate-500">
          {notesCount ? `${notesCount} nota(s)` : "Sin notas"}
        </span>
        <Button
          variant="outline"
          size="sm"
          className="h-7 rounded-lg px-2 text-[11px]"
          onClick={() => openQuickTaskPanel(task.id, "note")}
  ...
  function renderTaskDurationColumn(task: TaskItem) {
    ...
    return (
      <div className="min-w-0">
        ...
        <p className="truncate text-xs text-slate-500">
          {task.completedAt ? "Duración real" : "Tiempo transcurrido"}
        </p>
      </div>
    );
  }
  ...
  function renderTaskDeadlineColumn(task: TaskItem) {
    ...
    return (
      <span
        className={`inline-flex w-fit rounded-full border px-3 py-1 text-xs font-semibold ${deadline.badgeClass}`}
      >
        {deadline.label}
      </span>
    );
  ...
  return (
    <div className="space-y-6">
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <DropdownMenu>
              ...
              >
                <DropdownMenuItem
                  onSelect={() => openWorkspaceCreationDialog()}
                >
                  Crear proyecto
                </DropdownMenuItem>
                <DropdownMenuItem
                  onSelect={() => void openProjectDialog(selectedWorkspaceId)}
        ...
        stats={[
          {
            ...
            hint: "Contextos colaborativos visibles",
            tone: "sky",
          },
          {
            label: "No iniciadas",
            value: filteredTasks.filter(
              (task) => task.status === "OPEN" && !task.archivedAt,
            ).length,
            hint: "Pendiente de arrancar",
            tone: "amber",
          },
          {
            label: "En curso",
            value: filteredTasks.filter(
              (task) => task.status === "IN_PROGRESS" && !task.archivedAt,
            ).length,
            hint: "Ejecución activa",
            tone: "teal",
          },
          {
            label: "Finalizadas",
            value: filteredTasks.filter((task) => task.status === "DONE")
              .length,
            hint: "Cerradas",
            tone: "teal",
          },
      ...
      <div
        ...
        style={
          workspaceViewportHeight
            ? { height: `${workspaceViewportHeight}px` }
            : undefined
        }
      >
          ...
          <Card className="rounded-[26px] border-slate-200 shadow-[0_20px_40px_-32px_rgba(15,23,42,0.32)] xl:flex xl:h-full xl:min-h-0 xl:flex-col xl:overflow-hidden">
            <CardHeader className="border-b border-slate-100 pb-5">
              ...
              <CardDescription>
                ...
                administra sus listas cuando las necesites.
              </CardDescription>
              <div className="relative mt-4">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setWorkspaceListSearch(event.target.value)
                  }
            ...
            <CardContent className="min-h-0 space-y-3 p-4 md:p-5 xl:flex-1 xl:overflow-y-auto">
              ...
              filteredWorkspaces.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  No encontré proyectos con ese criterio.
                </p>
              ) : null}
              {filteredWorkspaces.map((workspace) => {
                ...
                return (
                  <div
                    ...
                    onDrop={(event) =>
                      void handleWorkspaceDrop(workspace.id, event)
                    }
                    className={
                      isSelected
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        >
                          <p className="font-semibold uppercase leading-5 text-slate-950">
                            {workspace.name}
                          </p>
                        </button>
                        <p
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                          ...
                          <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-emerald-800">
                            {formatRole(workspace.currentUserRole)}
                          </span>
                        </div>
                      </div>
                        ...
                        <DropdownMenu>
                          ...
                          >
                            ...
                            <DropdownMenuItem
                              onSelect={() => {
                                handleSelectWorkspace(workspace.id);
                                void openProjectDialog(workspace.id);
                              }}
                      ...
                      <div className="mt-4 space-y-3 border-t border-sky-200/70 pt-4">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="text-xs text-slate-500">
                              Cada lista ocupa todo el ancho y desde su botón +
                              puedes crear tareas o gestionarla.
                            </p>
                          </div>
                          <Button
                            type="button"
                            size="icon"
                            className="h-10 w-10 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700"
                            onClick={() => void openProjectDialog(workspace.id)}
                            disabled={!canManageCurrentWorkspace}
                          >
                        ...
                        <div className="grid gap-2">
                          <Button
                            ...
                            className={
                              !selectedProjectId
                                ? "justify-start rounded-2xl bg-slate-950 text-white hover:bg-slate-800"
                          ...
                          {workspace.projects.map((project) => (
                            <div
                              ...
                              onDragOver={(event) => {
                                event.preventDefault();
                                if (workspace.permissions?.canEditTasks)
                              ...
                              <div
                                className={
                                  selectedProjectId === project.id
                                    ? "flex items-center justify-between gap-3 border-b border-sky-200 bg-sky-600 px-4 py-2.5 text-white"
                                    : "flex items-center justify-between gap-3 border-b border-slate-200 bg-slate-100 px-4 py-2.5 text-slate-900"
                                }
                              >
                                <button
                                  type="button"
                                ...
                                <DropdownMenu>
                                  <DropdownMenuTrigger asChild>
                                    <Button
                                      ...
                                      className={
                                        selectedProjectId === project.id
                                          ? "h-8 w-8 rounded-full text-white hover:bg-white/15 hover:text-white"
                                          : "h-8 w-8 rounded-full text-slate-600 hover:bg-white hover:text-slate-900"
                                      }
                                  ...
                                  >
                                    <DropdownMenuItem
                                      onSelect={() =>
                                        openTaskCreationDialog(project.id)
                                      }
                                    ...
                                    <DropdownMenuItem
                                      onSelect={() =>
                                        openProjectDialog(workspace.id, project)
                                      }
                                      disabled={!canManageCurrentWorkspace}
                                    >
                              ...
                              <div className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm text-slate-600">
                                <div className="min-w-0">
                                  <p className="truncate text-xs text-slate-700">
                                    {project.description ||
                                      "Sin descripción operativa por ahora."}
                                  </p>
                                  <p className="mt-1 text-xs text-slate-500">
                                    {project._count?.tasks ?? 0} tarea(s)
                                  </p>
                                </div>
                                <Button
                                  type="button"
                                  ...
                                  className={
                                    selectedProjectId === project.id
                                      ? "h-8 rounded-xl bg-slate-950 px-3 text-xs text-white hover:bg-slate-800"
                        ...
                        {!workspace.projects.length ? (
                          <p className="text-xs text-slate-500">
                            Este proyecto aún no tiene listas. Puedes seguir
                            creando tareas directas o agregar una lista cuando
                            haga falta.
                          </p>
                        ) : null}
                        {selectedProjectId ? (
                          <p className="text-xs text-slate-500">
                            Puedes crear tareas desde el botón + de la lista
                            activa o desde el botón principal Crear tarea.
                          </p>
                        ) : null}
                      </div>
                    ) : null}
                ...
                <div className="flex flex-col gap-2 lg:items-end">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>
                        Todas las tareas de proyectos asignados
                      </TooltipContent>
                    </Tooltip>
                    {canManageWorkspace ? (
                      <Tooltip>
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>
                        Buscar por tarea, responsable, campo o descripción
                      </TooltipContent>
                    </Tooltip>
                    <Tooltip>
                      <TooltipTrigger asChild>
                      ...
                      >
                        <DropdownMenuCheckboxItem
                          ...
                          onCheckedChange={(checked) =>
                            setShowPriorityColumn(Boolean(checked))
                          }
                        >
                          Prioridad
                        </DropdownMenuCheckboxItem>
                        <DropdownMenuCheckboxItem
                          checked={showCreatedAtColumn}
                          onCheckedChange={(checked) =>
                            setShowCreatedAtColumn(Boolean(checked))
                          }
                        >
                          Fecha de creación
                        </DropdownMenuCheckboxItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuCheckboxItem
                          checked={visibleExtraTaskColumns.includes("duration")}
                          onCheckedChange={(checked) =>
                            toggleExtraTaskColumn("duration", Boolean(checked))
                          }
                        >
                          Duración (horas)
                        </DropdownMenuCheckboxItem>
                        <DropdownMenuCheckboxItem
                          checked={visibleExtraTaskColumns.includes("deadline")}
                          onCheckedChange={(checked) =>
                            toggleExtraTaskColumn("deadline", Boolean(checked))
                          }
                        >
                          Cumplimiento plazo
                        </DropdownMenuCheckboxItem>
                        <DropdownMenuCheckboxItem
                          checked={visibleExtraTaskColumns.includes(
                            "delay-days",
                          )}
                          onCheckedChange={(checked) =>
                            toggleExtraTaskColumn(
                              "delay-days",
                              Boolean(checked),
                            )
                          }
                        ...
                        <DropdownMenuCheckboxItem
                          checked={visibleExtraTaskColumns.includes("history")}
                          onCheckedChange={(checked) =>
                            toggleExtraTaskColumn("history", Boolean(checked))
                          }
                        >
                          Último cambio
                        </DropdownMenuCheckboxItem>
                        <DropdownMenuCheckboxItem
                          checked={visibleExtraTaskColumns.includes("note")}
                          onCheckedChange={(checked) =>
                            toggleExtraTaskColumn("note", Boolean(checked))
                          }
                        >
                          Nota rápida
                        </DropdownMenuCheckboxItem>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setTaskPeriodFilter(value as TaskPeriodFilter)
                      }
                    >
              ...
              <div className="flex h-full min-h-0 flex-col">
                        ...
                        >
                          ...
                          <CardContent className="space-y-2">
                            {tasksByStatus[status].length ? (
                              tasksByStatus[status].slice(0, 12).map((task) => (
                                ...
                                >
                                  ...
                                  <div className="mt-2 flex items-center justify-between text-[11px] text-slate-600">
                                    <span>
                                      {formatDate(task.dueAt, "Sin fecha")}
                                    </span>
                                    <span>
                              ...
                              <p className="text-xs text-slate-500">
                                No hay tareas en este estado para el periodo
                                seleccionado.
                              </p>
                            )}
                          </CardContent>
                ...
                {taskLayoutMode === "TABLE" ? (
                  <div className="min-h-0 flex-1 overflow-auto overscroll-contain">
                    <div
                      className="w-max min-h-full"
                      style={{ minWidth: `${taskTableMinWidth}px` }}
                    >
                      <div
                        className="grid gap-3 border-b border-slate-100 bg-white px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500 xl:sticky xl:top-0 xl:z-10"
                      ...
                      >
                        ...
                        {visibleExtraTaskColumns.includes("duration") ? (
                          <span>Duración (h)</span>
                        ) : null}
                        {visibleExtraTaskColumns.includes("deadline") ? (
                          <span>Cumplimiento plazo</span>
                        ) : null}
                        {visibleExtraTaskColumns.includes("delay-days") ? (
                          <span>Días demora</span>
                        ) : null}
                        {visibleExtraTaskColumns.includes("attachments") ? (
                          <span>Adjuntos</span>
                        ) : null}
                        {visibleExtraTaskColumns.includes("custom-fields") ? (
                          <span>Campos</span>
                        ) : null}
                        {visibleExtraTaskColumns.includes("history") ? (
                          <span>Último cambio</span>
                        ) : null}
                        {visibleExtraTaskColumns.includes("note") ? (
                      ...
                      {paginatedTasks.map((task) => {
                        ...
                        const canDeleteTask = Boolean(
                          canEditCurrentTask &&
                            ...
                            (task.status !== "CANCELED" || !task.archivedAt),
                        );
                        const pinned = isTaskPinned(task.id);
                        return (
                          <div
                            ...
                            style={{
                              ...
                              borderLeft: `5px solid ${normalizeHex(task.colorHex)}`,
                              borderBottomColor: "rgba(226,232,240,0.9)",
                            }}
                          >
                            {dragOverTaskId === task.id &&
                            dragOverTaskPlacement === "before" ? (
                              <div
                                className="pointer-events-none absolute -top-5 left-4 right-4 h-4 rounded-full border-2 border-dashed border-emerald-300 bg-emerald-100/85 shadow-[0_0_0_6px_rgba(110,231,183,0.16)] transition-all duration-300 ease-out animate-[pulse_1.6s_ease-in-out_infinite]"
                                aria-hidden="true"
                              />
                            ) : null}
                            <div className="flex min-w-0 items-center gap-2">
                              <GripVertical className="h-3.5 w-3.5 shrink-0 cursor-grab text-slate-400 active:cursor-grabbing" />
                              <div className="min-w-0 flex-1">
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                  ...
                                  <TooltipContent>
                                    <p className="max-w-sm break-words text-xs">
                                      {task.title}
                                    </p>
                                  </TooltipContent>
                                </Tooltip>
                              ...
                              <Button
                                ...
                                className={
                                  pinned
                                    ? "h-7 w-7 shrink-0 rounded-full text-amber-600 opacity-100 hover:bg-amber-100 hover:text-amber-700"
                                    : "h-7 w-7 shrink-0 rounded-full opacity-0 transition-opacity hover:bg-slate-200 hover:text-slate-900 group-hover:opacity-100"
                                }
                                aria-label={
                                  pinned
                                    ? `Desanclar ${task.title}`
                                    : `Anclar ${task.title}`
                                }
                            ...
                            <div className="overflow-hidden">
                              {renderTaskCreatedByColumn(task)}
                            </div>
                            <div className="overflow-hidden">
                            ...
                            <div className="truncate text-slate-600">
                              {task.completedAt
                                ? `Completada: ${formatDate(task.completedAt, "Sin fecha")}`
                                : formatDate(task.dueAt, "Sin fecha")}
                            </div>
                            {showCreatedAtColumn ? (
                              <div className="overflow-hidden">
                                {renderTaskCreatedAtColumn(task)}
                              </div>
                            ) : null}
                            {visibleExtraTaskColumns.includes("duration") ? (
                              <div className="overflow-hidden">
                                {renderTaskDurationColumn(task)}
                              </div>
                            ) : null}
                            {visibleExtraTaskColumns.includes("deadline") ? (
                              <div className="overflow-hidden">
                                {renderTaskDeadlineColumn(task)}
                              </div>
                            ) : null}
                            {visibleExtraTaskColumns.includes("delay-days") ? (
                              <div className="overflow-hidden">
                                {renderTaskDelayDaysColumn(task)}
                              </div>
                            ) : null}
                            {visibleExtraTaskColumns.includes("attachments") ? (
                              <div className="overflow-hidden">
                                {renderTaskAttachmentsColumn(task)}
                              </div>
                            ) : null}
                            {visibleExtraTaskColumns.includes(
                              "custom-fields",
                            ) ? (
                              <div className="overflow-hidden">
                                {renderTaskCustomFieldsColumn(task)}
                              </div>
                            ) : null}
                            {visibleExtraTaskColumns.includes("history") ? (
                              <div className="overflow-hidden">
                                {renderTaskHistoryColumn(task)}
                              </div>
                            ) : null}
                            {visibleExtraTaskColumns.includes("note") ? (
                              <div className="overflow-hidden">
                                {renderTaskNoteColumn(task)}
                              </div>
                            ) : null}
                            {dragOverTaskId === task.id &&
                            dragOverTaskPlacement === "after" ? (
                              <div
                                className="pointer-events-none absolute -bottom-5 left-4 right-4 h-4 rounded-full border-2 border-dashed border-emerald-300 bg-emerald-100/85 shadow-[0_0_0_6px_rgba(110,231,183,0.16)] transition-all duration-300 ease-out animate-[pulse_1.6s_ease-in-out_infinite]"
                                aria-hidden="true"
                              />
                            ) : null}
                            <div className="flex items-center gap-1 overflow-hidden">
                              ...
                              <DropdownMenu>
                                ...
                                >
                                  <DropdownMenuItem
                                    onSelect={() =>
                                      void handleUpdateTask(
                                        ...
                                        {
                                          title: task.archivedAt
                                            ? "Tarea restaurada"
                                            : "Tarea archivada",
                                          description: task.archivedAt
                                            ? "La tarea volvió a estar activa."
                                            : "La tarea se movió a archivadas.",
                                        },
                                      )
                                    }
                                  ...
                                  <DropdownMenuItem
                                    onSelect={() => openMoveTaskDialog(task)}
                                    disabled={!canEditCurrentTask}
                                  >
                                    Mover tarea
                                  </DropdownMenuItem>
                      ...
                      {!filteredTasks.length ? (
                        <div className="px-6 py-8 text-sm text-slate-500">
                          {taskViewMode === "MINE"
                            ? "No tienes tareas asignadas para mostrar."
                            : taskViewMode === "ALL_SPACES"
                              ? "No hay tareas para mostrar en tus proyectos asignados."
                              : selectedWorkspace
                                ? selectedProject
                                  ? "No hay tareas para mostrar en esta lista."
                                  : selectedWorkspace.projects.length
                                    ? "No hay tareas para mostrar en este proyecto."
                                    : "Todavía no hay tareas en este proyecto. Puedes crear una tarea directa o agregar una lista."
                                : "Selecciona un proyecto para ver tareas o usa Crear tarea para registrar una nueva."}
                        </div>
                      ) : null}
                    </div>
                  </div>
                ...
                {taskLayoutMode === "TABLE" ? (
                  <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50/90 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex flex-wrap items-center gap-3 text-xs text-slate-600">
                      <span>
                        {filteredTasks.length
                          ? `Mostrando ${visibleTaskRange.start}-${visibleTaskRange.end} de ${filteredTasks.length} tareas`
                          : "Sin tareas para paginar"}
                      </span>
      ...
      <Dialog
        open={taskCancelDialogOpen}
        onOpenChange={(open) => {
          if (!open) closeTaskCancellationDialog();
        }}
      >
        <DialogContent className="max-w-lg">
          ...
          <div className="grid gap-3 py-2">
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
              Esta acción ya no borra la tarea físicamente. Se guardará la traza
              en historial y se notificará a los involucrados.
            </div>
            <div className="grid gap-2">
              <Label>
                Motivo
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={closeTaskCancellationDialog}
              disabled={cancellingTask}
            >
              Cancelar
            </Button>
            <Button
              onClick={() => void handleConfirmTaskCancellation()}
              disabled={cancellingTask}
            >
      ...
      <Dialog
        open={Boolean(quickTaskPanel)}
        onOpenChange={(open) => {
          if (!open) setQuickTaskPanel(null);
        }}
      >
        <DialogContent
          className={quickTaskPanel?.mode === "note" ? "max-w-md" : "max-w-2xl"}
        >
          <DialogHeader>
            <DialogTitle>
          ...
          {quickTaskPanel?.mode === "attachments" ? (
            <div className="grid gap-3 sm:grid-cols-2">
              {quickTask?.attachmentsJson?.length ? (
                quickTask.attachmentsJson.map((attachment) => (
                  <div
                    key={attachment.id}
          ...
          {quickTaskPanel?.mode === "custom-fields" ? (
            <div className="space-y-3">
              {quickTask?.customFieldsJson?.length ? (
                quickTask.customFieldsJson.map((field) => (
                  <div
                    key={field.id}
                    className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3"
                  >
                ...
                <p className="text-sm text-slate-500">
                  Esta tarea no tiene campos personalizados.
                </p>
              )}
            </div>
          ) : null}
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 text-sm text-slate-600">
                <div className="flex items-center justify-between gap-3">
                  <p className="font-medium text-slate-900">
                    {translateTaskHistoryMessage(
                      quickTaskLatestHistory.message,
                      quickTaskLatestHistory.type,
                    )}
                  </p>
                  <span className="text-xs text-slate-500">
                    {formatDate(quickTaskLatestHistory.createdAt, "Sin fecha")}
                  </span>
                </div>
                <p className="mt-2 text-xs text-slate-500">
                  {quickTaskLatestHistory.actorUser?.name ||
                    quickTaskLatestHistory.actorUser?.email ||
                    "Sistema"}{" "}
                  · {formatTaskHistoryType(quickTaskLatestHistory.type)}
                </p>
              </div>
            ) : (
              <p className="text-sm text-slate-500">
                Esta tarea no tiene historial todavía.
              </p>
            )
          ) : null}
          ...
          {quickTaskPanel?.mode === "note" ? (
            <div className="space-y-3">
              <Textarea
                ...
                placeholder="Escribe una nota breve para esta tarea..."
                rows={4}
                disabled={!canEditTasks}
              />
              <div className="flex justify-end gap-2">
                <Button
                  variant="outline"
                  onClick={() => setQuickTaskPanel(null)}
                >
                  Cancelar
                </Button>
                <Button
                  onClick={() => void handleQuickAddNote()}
                  disabled={savingQuickNote || !canEditTasks}
                >
                  {savingQuickNote ? "Guardando..." : "Guardar nota"}
                </Button>
              </div>
            </div>
          ) : null}
      ...
      >
        <DialogContent className="max-h-[90vh] max-w-[880px] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Administrar proyecto</DialogTitle>
            <DialogDescription>
              Define privacidad, miembros y responsables sin afectar las tareas
              ya existentes.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 py-1.5">
            <div className="grid gap-3 md:grid-cols-2">
              <div className="grid gap-1.5">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setWorkspaceSettingsForm((current) => ({
                      ...current,
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setWorkspaceSettingsForm((current) => ({
                      ...
                      members:
                        value !== "__none__" &&
                        !current.members.some((item) => item.userId === value)
                          ? [
                              ...current.members,
                              { userId: value, role: "MANAGER" },
                            ]
                          : current.members,
                    }))
                  }
            ...
            <div className="grid gap-1.5">
              ...
              <Textarea
                value={workspaceSettingsForm.description}
                onChange={(event) =>
                  setWorkspaceSettingsForm((current) => ({
                    ...current,
            ...
            <div className="grid gap-1.5">
              ...
              <Select
                value={workspaceSettingsForm.visibility}
                onValueChange={(value) =>
                  setWorkspaceSettingsForm((current) => ({
                    ...current,
              ...
              <p className="text-xs text-slate-500">
                Público permite visibilidad general de la empresa. Privado y
                oculto conservan acceso por miembros.
              </p>
            </div>
            {workspaceSettingsForm.scope === "SEDE" ? (
              <div className="grid gap-1.5">
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    setWorkspaceSettingsForm((current) => ({
                      ...current,
                      sedeIds: current.sedeIds.includes(value)
                        ? current.sedeIds
                        : [...current.sedeIds, value],
                    }));
                  }}
                  disabled={!canManageWorkspace}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Selecciona y agrega sedes" />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <div className="flex flex-wrap gap-2">
                  {workspaceSettingsForm.sedeIds.length ? (
                    workspaceSettingsForm.sedeIds.map((sedeId) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            setWorkspaceSettingsForm((current) => ({
                              ...current,
                              sedeIds: current.sedeIds.filter(
                                (item) => item !== sedeId,
                              ),
                            }))
                          }
            ...
            <div className="grid gap-3 lg:grid-cols-[0.95fr_1.05fr]">
              <div className="grid gap-1.5">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setWorkspaceMemberSearch(event.target.value)
                  }
                  placeholder="Busca usuarios para invitarlos al proyecto..."
                  disabled={!canManageWorkspace}
                />
                <div className="max-h-52 space-y-1.5 overflow-y-auto rounded-xl border border-slate-200 p-2.5">
                  ...
                  {!workspaceMemberCandidates.length ? (
                    <p className="text-sm text-slate-400">
                      No hay usuarios adicionales con ese filtro.
                    </p>
                  ) : null}
                </div>
              </div>
              <div className="grid gap-1.5">
                ...
                <div className="max-h-64 space-y-1.5 overflow-y-auto rounded-xl border border-slate-200 p-2.5">
                  {workspaceSettingsForm.members.map((member) => {
                    ...
                    const locked =
                      member.userId === selectedWorkspace?.createdBy?.id ||
                      member.userId === workspaceSettingsForm.ownerUserId;
                    return (
                      <div
                        key={member.userId}
                        className="grid gap-1.5 rounded-xl border border-slate-200 bg-slate-50/70 p-2.5 md:grid-cols-[1fr_160px_96px] md:items-center"
                      >
                        <div>
                          <p className="font-medium text-slate-950">
                            {getUserLabel(user)}
                          ...
                          <p className="text-xs text-slate-500">
                            {locked
                              ? "Rol protegido por propiedad del proyecto"
                              : "Puedes cambiar el rol o quitar el acceso"}
                          </p>
                        </div>
                        <Select
                          value={member.role}
                            ...
                            setWorkspaceSettingsForm((current) => ({
                              ...
                              members: current.members.map((item) =>
                                item.userId === member.userId
                                  ? { ...item, role: value as WorkspaceRole }
                                  : item,
                              ),
                            }))
                          }
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={savingWorkspace || !canManageWorkspace}
            >
      ...
      <Dialog open={workspaceDialogOpen} onOpenChange={setWorkspaceDialogOpen}>
        <DialogContent className="max-w-[760px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Define privacidad, responsable y miembros. Las tareas podrán
              relacionarse a este proyecto de forma opcional.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 py-1.5">
            <div className="grid gap-1.5">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              {workspaceForm.scope === "SEDE" ? (
                <div className="grid gap-1.5 md:col-span-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      if (value === "__none__") return;
                      setWorkspaceForm((current) => {
                        const nextSedeIds = current.sedeIds.includes(value)
                          ? current.sedeIds
                          : [...current.sedeIds, value];
                        return {
                          ...current,
                          sedeId: nextSedeIds[0] || value,
                          sedeIds: nextSedeIds,
                        };
                      });
                    }}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {workspaceForm.sedeIds.length ? (
                      workspaceForm.sedeIds.map((sedeId) => {
                        const sede = sedes.find((item) => item.id === sedeId);
                        return (
                          <button
                            key={sedeId}
                            ...
                            onClick={() =>
                              setWorkspaceForm((current) => {
                                const nextSedeIds = current.sedeIds.filter(
                                  (item) => item !== sedeId,
                                );
                                return {
                                  ...current,
                                  sedeId: nextSedeIds[0] || "",
                                  sedeIds: nextSedeIds,
                                };
                              })
                            }
                      ...
                      <span className="text-sm text-slate-400">
                        Agrega una o varias sedes para este proyecto.
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="grid gap-1.5 md:col-span-2">
                  ...
                  <Select
                    value={workspaceForm.ownerUserId || "__none__"}
                    onValueChange={(value) =>
                      setWorkspaceForm((current) => ({
                        ...current,
                        ownerUserId: value === "__none__" ? "" : value,
            ...
            <div className="grid gap-1.5">
              ...
              <div className="max-h-40 space-y-1.5 overflow-y-auto rounded-xl border border-slate-200 p-2.5">
                {workspaceCandidates.map((user) => {
                  const selected = workspaceForm.memberUserIds.includes(
                    user.id,
                  );
                  return (
                    <button
                      ...
                      className={
                        selected
                          ? "flex w-full items-center justify-between rounded-lg border border-sky-300 bg-sky-50 px-3 py-2 text-left"
                          : "flex w-full items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-2 text-left"
                      }
                    >
                      <span>{getUserLabel(user)}</span>
                      <span className="text-xs text-slate-500">
                        {selected ? "Invitado" : "Agregar"}
                      </span>
                    </button>
                  );
                })}
          ...
          <DialogFooter>
            ...
            >
              {savingWorkspace ? "Guardando..." : "Crear proyecto"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open) setTaskRelationOptionsOpen(false);
        }}
      >
        <DialogContent className="max-h-[90vh] max-w-[760px] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Crea una tarea directa y relaciónala de forma opcional con un
              proyecto o una lista existente.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 py-1.5">
            <div className="grid gap-1.5">
            ...
            <div className="flex justify-end">
              <Button
                ...
                onClick={() =>
                  setTaskRelationOptionsOpen((current) => !current)
                }
              >
                {taskRelationOptionsOpen ? "Ocultar opcional" : "Opcional"}
              </Button>
            </div>
            {taskRelationOptionsOpen ? (
              <div className="space-y-3">
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="grid gap-1.5">
                    <Label>Relacionar con proyecto</Label>
                    <Select
                      value={taskForm.workspaceId || "__none__"}
                      onValueChange={(value) =>
                        setTaskForm((current) => ({
                          ...current,
                          workspaceId: value === "__none__" ? "" : value,
                          projectId:
                            value === "__none__" ? "" : current.projectId,
                        }))
                      }
                    >
                      <SelectTrigger>
                  ...
                  <div className="grid gap-1.5">
                    <Label>Relacionar con lista</Label>
                    <Select
                      value={taskForm.projectId || "__none__"}
                      onValueChange={(value) =>
                        setTaskForm((current) => ({
                          ...current,
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        {(taskFormWorkspace?.projects || []).map((project) => (
              ...
              <div className="flex flex-wrap items-center gap-2.5">
                {COLOR_PRESETS.map((color) => (
                  <button
                    ...
                    type="button"
                    className={
                      normalizeHex(taskForm.colorHex) === color
                        ? "h-9 w-9 rounded-full ring-4 ring-slate-300"
                ...
                <Input
                  type="color"
                  value={normalizeHex(taskForm.colorHex)}
                  onChange={(event) =>
                    setTaskForm((current) => ({
                      ...current,
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(135deg, ${normalizeHex(taskForm.colorHex)} 0%, #ffffff 120%)`,
                }}
              >
                ...
                <p className="mt-1 text-sm text-white/85">
                  ...
                  {PRIORITY_META[taskForm.priority].label}
                </p>
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setTaskForm((current) => ({
                          ...current,
                          assignedToUserIds: selected
                            ? current.assignedToUserIds.filter(
                                (item) => item !== user.id,
                              )
            ...
            <div className="grid gap-2 rounded-xl border border-slate-200 p-3">
              <div>
                <Label>Adjuntos de la tarea</Label>
                <p className="text-xs text-slate-500">
                  Selecciona los archivos que se subirán al crear la tarea.
                </p>
              </div>
              <input
                ...
                onChange={(event) => {
                  ...
                  setCreateTaskFiles((current) => [...current, ...files]);
                  event.target.value = "";
                }}
              />
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-slate-500">
                  {createTaskFiles.length} archivo(s) seleccionado(s)
                </span>
              </div>
              {createTaskFiles.length ? (
            ...
            <div className="grid gap-3 rounded-xl border border-slate-200 p-3">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Agrega información adicional de texto o archivo.
                </p>
              </div>
              <div className="grid gap-2 md:grid-cols-[1fr_150px_1fr_auto]">
                <Input
                  value={createTaskCustomFieldDraft.label}
                  onChange={(event) =>
                    setCreateTaskCustomFieldDraft((current) => ({
                      ...current,
                ...
                />
                <Select
                  value={createTaskCustomFieldDraft.type}
                  onValueChange={(value) =>
                    setCreateTaskCustomFieldDraft((current) => ({
                      ...current,
                      type: value as TaskCustomFieldType,
                      textValue: "",
                      file: null,
                    }))
                  }
                ...
                {createTaskCustomFieldDraft.type === "TEXT" ? (
                  <Input
                    value={createTaskCustomFieldDraft.textValue}
                    onChange={(event) =>
                      setCreateTaskCustomFieldDraft((current) => ({
                        ...current,
                  ...
                  <>
                    <input
                      ref={createCustomFieldFileInputRef}
                      type="file"
                      accept={attachmentAccept()}
                      className="hidden"
                      onChange={(event) =>
                    ...
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => createCustomFieldFileInputRef.current?.click()}
                    >
                      {createTaskCustomFieldDraft.file?.name || "Elegir archivo"}
                    </Button>
                  </>
                )}
                <Button type="button" onClick={handleAddCreateTaskCustomField}>
                  Agregar campo
                </Button>
              </div>
              {createTaskCustomFields.map((field) => (
                <div
                  key={field.id}
                ...
                >
                  <span className="min-w-0 truncate">
                    ...
                    {field.type === "TEXT" ? field.textValue : field.file?.name}
                  </span>
                  <Button
                    type="button"
                    variant="outline"
            ...
            <div className="grid gap-1.5 rounded-xl border border-slate-200 p-3">
              <Label>Crear nota</Label>
              <Textarea
                value={createTaskNote}
                onChange={(event) => setCreateTaskNote(event.target.value)}
                rows={3}
                placeholder="Escribe una nota inicial para el historial de la tarea..."
              />
            </div>
          </div>
          <DialogFooter>
            <Button
              ...
              onClick={() => {
                setTaskDialogOpen(false);
                setTaskRelationOptionsOpen(false);
              }}
            >
              Cancelar
            </Button>
            <Button
              ...
              disabled={savingTask || !canCreateTask}
            >
      ...
      <Dialog open={detailDialogOpen} onOpenChange={setDetailDialogOpen}>
        <DialogContent className="max-w-5xl max-h-[94vh] overflow-hidden p-0">
          <DialogHeader className="sticky top-0 z-10 border-b bg-white px-6 py-4">
            ...
            <p className="text-sm text-slate-500">
              ...
              {formatDate(detailForm.dueAt, "Sin fecha")}
            </p>
          </DialogHeader>
          <input
          ...
          />
          <div className="grid max-h-[calc(94vh-170px)] auto-rows-max gap-4 overflow-y-auto px-6 py-4">
            <Card className="min-h-max overflow-hidden border-0 shadow-none">
              <CardContent
                className="rounded-[28px] border p-5"
                style={{
                  background: `radial-gradient(circle at top right, ${normalizeHex(detailForm.colorHex)}33, transparent 30%), linear-gradient(180deg, #ffffff 0%, #f8fafc 100%)`,
                  borderColor: `${normalizeHex(detailForm.colorHex)}55`,
                }}
              >
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        {COLOR_PRESETS.map((color) => (
                          <button
                            ...
                            type="button"
                            className={
                              normalizeHex(detailForm.colorHex) === color
                                ? "h-9 w-9 rounded-full ring-4 ring-slate-300"
                                : "h-9 w-9 rounded-full ring-1 ring-slate-200"
                            }
                            style={{ backgroundColor: color }}
                        ...
                        <Input
                          type="color"
                          value={normalizeHex(detailForm.colorHex)}
                          onChange={(event) =>
                            setDetailForm((current) => ({
                              ...current,
                              colorHex: event.target.value.toUpperCase(),
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <span
                    className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${STATUS_META[detailForm.status].badgeClass}`}
                  >
                    {STATUS_META[detailForm.status].label}
                  </span>
                  <span
                    className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${PRIORITY_META[detailForm.priority].badgeClass}`}
                  >
                    {PRIORITY_META[detailForm.priority].label}
                  </span>
                  <span className="inline-flex rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700">
                    Creada: {formatDate(selectedTask?.createdAt, "Sin fecha")}
                  </span>
                  <span className="inline-flex rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700">
                    Actualizada:{" "}
                    {formatDate(selectedTask?.updatedAt, "Sin fecha")}
                  </span>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="grid gap-4 md:grid-cols-[220px_1fr]">
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      setDetailAssigneeSearch(event.target.value)
                    }
                ...
                <div className="grid gap-3">
                  ...
                  <div className="max-h-36 space-y-2 overflow-y-auto rounded-2xl border border-slate-200 p-3">
                    {detailAssigneeCandidates.map((user) => {
                      const selected = detailForm.assignedToUserIds.includes(
                        user.id,
                      );
                      return (
                        <button
                          ...
                          className={
                            selected
                              ? "flex w-full items-center justify-between rounded-xl border border-sky-300 bg-sky-50 px-3 py-2 text-left"
                              : "flex w-full items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2 text-left"
                          }
                          disabled={!canEditTasks}
                        >
                          <span>{getUserLabel(user)}</span>
                          <span className="text-xs text-slate-500">
                            {selected ? "Asignado" : "Agregar"}
                          </span>
                        </button>
                      );
                    })}
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Sube imágenes, audios, videos o documentos que sirvan como
                  evidencia o referencia directa de la tarea.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex flex-wrap items-center gap-3">
                  <Button
                    ...
                    disabled={!canEditTasks || uploadingAttachment}
                  >
                  ...
                  <Button
                    variant="outline"
                    onClick={() => setExternalAttachmentDialogOpen(true)}
                    disabled={!canEditTasks || uploadingAttachment}
                  >
                    Vincular Drive/OneDrive
                  </Button>
                  <span className="text-sm text-slate-500">
                    {detailForm.attachmentsJson.length} archivo(s) vinculados
                  </span>
                </div>
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {detailForm.attachmentsJson.map((attachment) => (
                    <div
                      key={attachment.id}
                      className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className="font-medium text-slate-950 line-clamp-1">
                            {attachment.name}
                          </p>
                          <p className="mt-1 text-xs text-slate-500">
                            {attachment.type.toUpperCase()} ·{" "}
                        ...
                        {canEditTasks ? (
                          <Button
                            ...
                            onClick={() =>
                              setDetailForm((current) => ({
                                ...current,
                                attachmentsJson: current.attachmentsJson.filter(
                      ...
                      <div className="mt-3 rounded-xl border border-slate-200 bg-white p-2">
                        ...
                        {attachment.type === "document" ? (
                          <div className="flex h-40 items-center justify-center rounded-lg bg-slate-100 text-sm text-slate-600">
                  ...
                  {!detailForm.attachmentsJson.length ? (
                    <p className="text-sm text-slate-400">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Agrega campos de texto o de archivo y luego edítalos o
                  elimínalos individualmente.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid gap-3 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 md:grid-cols-[1fr_160px_1fr_140px] md:items-end">
                  <div className="grid gap-2">
                  ...
                  {customFieldDraft.type === "TEXT" ? (
                    ...
                    <div className="grid gap-2">
                      ...
                      <Button
                        variant="outline"
                        onClick={() => {
                          setCustomFieldUploadTarget("new");
                          customFieldFileInputRef.current?.click();
                        }}
                        disabled={!canEditTasks || uploadingAttachment}
                      >
                        {customFieldDraft.file
                          ? "Reemplazar archivo"
                          : uploadingAttachment
                            ? "Subiendo..."
                            : "Subir archivo"}
                ...
                <div className="space-y-3">
                  {detailForm.customFieldsJson.map((field) => (
                    <div
                      key={field.id}
                      className="rounded-2xl border border-slate-200 bg-white p-4"
                    >
                      <div className="grid gap-3 lg:grid-cols-[1fr_160px_1fr_110px] lg:items-start">
                        <div className="grid gap-2">
                          <Label>Etiqueta</Label>
                          <Input
                            value={field.label}
                            onChange={(event) =>
                              setDetailForm((current) => ({
                                ...current,
                                customFieldsJson: current.customFieldsJson.map(
                                  (item) =>
                                    item.id === field.id
                            ...
                            <Input
                              ...
                              onChange={(event) =>
                                setDetailForm((current) => ({
                                  ...current,
                                  customFieldsJson:
                                    current.customFieldsJson.map((item) =>
                                      item.id === field.id
                                        ? {
                                            ...item,
                                            textValue: event.target.value,
                          ...
                          <div className="grid gap-2">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              <Button
                                variant="outline"
                                onClick={() => {
                                  setCustomFieldUploadTarget(field.id);
                                  customFieldFileInputRef.current?.click();
                                }}
                                disabled={!canEditTasks || uploadingAttachment}
                              >
                                {field.file ? "Reemplazar" : "Subir"}
                              </Button>
                              {field.file ? (
                                <Button
                                  variant="outline"
                          ...
                          <Button
                            ...
                            onClick={() =>
                              setDetailForm((current) => ({
                                ...current,
                                customFieldsJson:
                                  current.customFieldsJson.filter(
                  ...
                  {!detailForm.customFieldsJson.length ? (
                    <p className="text-sm text-slate-400">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {visibleTaskHistoryEntries.length ? (
                  visibleTaskHistoryEntries.map((entry) => (
                    <div
                      key={entry.id}
                      className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3 text-sm text-slate-600"
                    >
                      <div className="flex items-center justify-between gap-3">
                        <p className="font-medium text-slate-900">
                          {translateTaskHistoryMessage(
                        ...
                        <span className="text-xs text-slate-500">
                          {formatDate(entry.createdAt, "Sin fecha")}
                        </span>
                      </div>
                      <p className="mt-1 text-xs text-slate-500">
                        {getUserLabel(entry.actorUser)} ·{" "}
                        {formatTaskHistoryType(entry.type)}
                      </p>
                    </div>
                  ))
                ) : (
                ...
                {selectedTaskHistory.length > 5 ? (
                  <div className="pt-1">
                    {canLoadMoreTaskHistory ? (
                      <Button
                        variant="outline"
                        className="rounded-xl"
                        onClick={() =>
                          setDetailHistoryVisibleCount((current) =>
                            Math.min(current + 5, selectedTaskHistory.length),
                          )
                        }
                      >
                        Cargar más
                      </Button>
                    ) : canCollapseTaskHistory ? (
                      <Button
                        variant="outline"
                        className="rounded-xl"
                        onClick={() => setDetailHistoryVisibleCount(5)}
                      >
            ...
            <Card>
              <CardContent className="grid gap-4 p-4 md:grid-cols-[160px_1fr_140px] md:items-start">
                ...
                <Button
                  ...
                  disabled={savingNote || !canEditTasks}
                >
            ...
            <Card>
              <CardContent className="flex items-center justify-between gap-3 p-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Puedes archivar la tarea sin perder historial, adjuntos ni
                    responsables.
                  </p>
                </div>
                <Button
          ...
          <DialogFooter className="sticky bottom-0 z-10 border-t bg-white px-6 py-4">
            ...
            <Button
              ...
              disabled={savingDetail || !canEditTasks}
            >
      ...
      <Dialog open={taskMoveDialogOpen} onOpenChange={setTaskMoveDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Mover tarea</DialogTitle>
            <DialogDescription>
              Relaciona la tarea con otro proyecto y, si hace falta, con una
              lista de destino.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 py-2">
            <div className="grid gap-2">
              ...
              <Select
                value={taskMoveForm.workspaceId || "__none__"}
                onValueChange={(value) => {
                  const nextWorkspace =
                    editableWorkspaces.find(
                      (workspace) => workspace.id === value,
                    ) ?? null;
                  setTaskMoveForm((current) => ({
                    ...current,
                    workspaceId: value === "__none__" ? "" : value,
                    projectId: nextWorkspace?.projects.some(
            ...
            <div className="grid gap-2">
              ...
              <Select
                value={taskMoveForm.projectId || "__none__"}
                onValueChange={(value) =>
                  setTaskMoveForm((current) => ({
                    ...current,
                    projectId: value === "__none__" ? "" : value,
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="__none__">Sin lista</SelectItem>
                  {selectedMoveWorkspace?.projects.map((project) => (
                    <SelectItem key={project.id} value={project.id}>
                      {project.name}
                    </SelectItem>
                  ))}
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setTaskMoveDialogOpen(false)}
            >
              Cancelar
            </Button>
            <Button onClick={() => void handleMoveTask()} disabled={movingTask}>
              {movingTask ? "Moviendo..." : "Mover tarea"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      <Dialog open={projectDialogOpen} onOpenChange={setProjectDialogOpen}>
        <DialogContent className="max-w-[680px]">
          <DialogHeader>
            <DialogTitle>
              {projectForm.projectId ? "Editar lista" : "Crear lista"}
            </DialogTitle>
            <DialogDescription>
              Define el nombre, la descripción y, si hace falta, el proyecto de
              destino de la lista.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 py-1.5">
            <div className="grid gap-1.5">
              ...
              <Select
                value={projectForm.workspaceId || "__none__"}
                onValueChange={(value) =>
            ...
            <div className="grid gap-1.5">
              ...
              <Input
                ...
                placeholder="Pendientes comerciales"
              />
            </div>
            <div className="grid gap-1.5">
              <Label>Descripción</Label>
              <Textarea
                ...
                rows={4}
                placeholder="Objetivo, entregables o contexto operativo de la lista"
              />
            </div>
          </div>
          <DialogFooter>
      ...
      <CrmFileLibraryPicker
        ...
        title="Seleccionar archivo del repositorio CRM"
        allowFolders={false}
      />
      <Dialog
        open={externalAttachmentDialogOpen}
        onOpenChange={setExternalAttachmentDialogOpen}
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Vincular desde Drive o OneDrive</DialogTitle>
            <DialogDescription>
              Pega una URL compartida para registrarla como adjunto externo de
              esta tarea.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 py-2">
            <div className="grid gap-2">
              ...
              <Select
                value={externalAttachmentForm.provider}
                onValueChange={(value) =>
                  setExternalAttachmentForm((current) => ({
                    ...current,
                    provider: value as ExternalAttachmentProvider,
                  }))
                }
              >
                <SelectTrigger>
            ...
            <div className="grid gap-2">
              ...
              <Input
                value={externalAttachmentForm.name}
                onChange={(event) =>
                  setExternalAttachmentForm((current) => ({
                    ...current,
                    name: event.target.value,
                ...
                placeholder="Propuesta comercial Q2"
              />
            </div>
            <div className="grid gap-2">
              <Label>URL compartida</Label>
              <Input
                value={externalAttachmentForm.url}
                onChange={(event) =>
                  setExternalAttachmentForm((current) => ({
                    ...current,
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => void handleExternalAttachmentLink()}
              disabled={uploadingAttachment}
            >
              {uploadingAttachment ? "Vinculando..." : "Vincular adjunto"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
