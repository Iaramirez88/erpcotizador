...
import { Sparkles, LoaderCircle, ClipboardCopy, ArrowRight } from "lucide-react"
...
export function LitografiaAiAssistant(props: {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const json = (await res.json().catch(() => null)) as
        | { ok?: boolean; error?: string; data?: LitografiaAiResult; connection?: LitografiaAiConnection; pricing?: LitografiaAiPricing }
        | null
  ...
  const handleApplyToClassic = () => {
    if (!result || !props.onApplyToClassic) return
