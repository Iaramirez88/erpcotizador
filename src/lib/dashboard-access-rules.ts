import { AccessLevel, ModuleKey, type SedeRole } from '@prisma/client'
...
export function deriveExplicitCapabilityLevel(args: {
  ...
  for (const action of definition.actions) {
    ...
    if (!grant) continue
    hasAnyGrant = true
    if (grant.allowed) allowedLevels.add(capabilityActionToAccessLevel(action))
  }
...
export function buildAllowedDashboardPermissionKeysFromContext(args: {
  context: DashboardAccessContext
  ...
  for (const rule of DASHBOARD_PERMISSION_RULES) {
    const canRead = canAccessDashboardRuleFromContext({ context: args.context, rule })
    if (!canRead) continue

    const allowedByBase = !baseSet || rule.hrefs.some((href) => href === '/dashboard' || baseSet.has(href))
    if (allowedByBase || rule.directGrantOnly) keys.add(rule.key)
  }

  return [...keys]
}

export function buildAllowedDashboardHrefsFromContext(args: {
  context: DashboardAccessContext
  baseAllowedHrefs?: string[] | null
}) {
  const baseSet = args.baseAllowedHrefs?.length ? new Set(args.baseAllowedHrefs) : null
  ...
  for (const item of DASHBOARD_NAV_CATALOG) {
    ...
    if (!rules.length) {
      ...
      const level = args.context.legacyAccess[moduleKey as ModuleKey] ?? 'NONE'
      if (level !== 'NONE') hrefs.add(item.href)
      continue
    }

    const canReadRules = rules.filter((rule) => canAccessDashboardRuleFromContext({ context: args.context, rule }))
