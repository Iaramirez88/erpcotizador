 ...
 * - Se asegura que sgdigitalnet@gmail.com exista (si aplica), quede como ADMIN
 *   en la sede principal y como planOwnerUserId (si está vacío).
 *
 * Seguridad:
...
async function main() {
	...
	console.log(
		'Alcance:',
		migrateAll ? 'ALL (todo usuario fuera de la empresa destino)' : 'PERSONAL (solo PERS- / sin empresa)'
	)
	console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
	...
	const users = await prisma.user.findMany({
		where: {
			...
			...(migrateAll
				...
				: {
						OR: [{ empresaId: null }, { empresa: { nit: { startsWith: 'PERS-' } } }],
					}),
		},
		select: {
			id: true,
	...
	const candidates = users.filter((u) => u.empresaId !== targetEmpresa.id && u.email !== adminEmail)
	...
	console.log('Nota: puede que los usuarios deban cerrar sesión y volver a iniciar para reflejar cambios en UI.')
}
